...
void slimbus_hi6402_param_init(slimbus_device_info_t *dev)
{
	...
	for (track_type = 0; track_type < SLIMBUS_TRACK_MAX; track_type++) {
		...
		slimbus_drv_get_params_la(track_type, &source_la, &sink_la, &tp);

		pchprop = track_config_table[track_type].channel_pro;
				...
				slimbus_track_param_t *params)
{
	...
	if ((SLIMBUS_TRACK_VOICE_DOWN == track_type) || (SLIMBUS_TRACK_VOICE_UP == track_type)) {
		...
		track_config_table[track].params.channels = 2;
		pr_info("[%s:%d] track type:%d, rate:%d, chnum:%d\n", __FUNCTION__, __LINE__, track, track_config_table[track_type].params.rate, track_config_table[track].params.channels);

		for (ch = 0; ch < track_config_table[track].params.channels; ch++) {
			pchprop->sd 	   = sd_table[rf][track][ch];
			pchprop->pr 	   = pr_table[rf][track];

			if (params->rate == SLIMBUS_SAMPLE_RATE_16K) {
				pchprop->sd 	   = sd_voice_down_16k[rf][ch];
				pr_table[rf][track] = SLIMBUS_PR_16K;
			}
		...
		for (ch = 0; ch < track_config_table[track].params.channels; ch++) {
			...
			if (params->rate == SLIMBUS_SAMPLE_RATE_48K) {
				pchprop->sd 	   = sd_soundtrigger_48k[ch];
				pr_table[rf][track] = SLIMBUS_PR_48K;
...
void release_hi6402_slimbus_device(slimbus_device_info_t *device)
{
	if (device != NULL) {
...
EXPORT_SYMBOL(slimbus_hi6402_get_st_params);
