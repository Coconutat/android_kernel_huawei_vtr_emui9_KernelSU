...
static int bfm_check_validity_of_bootfail_log_in_dfx(bfm_bfi_member_info_t *pbfi_info, u64 rtc_time, int* log_idx)
{
    char *pdata_buf = NULL;
    ...
    seek_result = sys_lseek(fd, BFM_BFI_HEADER_SIZE + BFM_BFI_MEMBER_SIZE*(*log_idx), SEEK_SET);
    ...
    for (i = (*log_idx); i < BFM_BFI_RECORD_TOTAL_COUNT; i++)
    {
        ...
        {
            *log_idx = i +1;
            BFMR_PRINT_KEY_INFO("RTC time in BFI[%d]: %llx, pbfi_info->is_log_saved BFI: %llx\n", i, pbfi_info->rtcValue, pbfi_info->is_log_saved);
            ret = 0;
            break;
        }
        BFMR_PRINT_KEY_INFO("RTC time in BFI[%d]: %llx, pbfi_info->is_log_saved BFI: %llx\n", i, pbfi_info->rtcValue, pbfi_info->is_log_saved);
...
static int bfm_update_save_flag_in_bfi(bfm_bfi_member_info_t *pbfi_info)
{
    ...
    for (i = 0; i < BFM_BFI_RECORD_TOTAL_COUNT; i++)
    {
        ...
        if ((pbfi_info->rtcValue == pbfi_info_local->rtcValue) &&  (pbfi_info_local->is_log_saved != BFM_BFI_SAVED))
        {
...
//    pbfi_info = (bfm_bfi_member_info_t *)bfmr_malloc(sizeof(bfm_bfi_member_info_t));
    ...
    while (0 == bfm_check_validity_of_bootfail_log_in_dfx(pbfi_info, 0, &log_idx))
    {
...
static int bfm_save_pwrkey_press_flag_in_bfi(bfm_process_bootfail_param_t *pparam)
{
    bfm_bfi_member_info_t *pbfi_info_local = NULL;
    char *bfi_part_full_path = NULL;
    int ret = -1;
    ...
    ret = bfmr_write_emmc_raw_part(bfi_part_full_path, BFM_BFI_PWK_PRESS_OFF, (char *)pbfi_info_local, BFM_BFI_MEMBER_SIZE);
...
int bfm_update_platform_logs(bfm_bootfail_log_info_t *pbootfail_log_info)
{
    ...
    snprintf(dst_file_path, BFMR_MAX_PATH - 1, "%s/%s", pbootfail_log_info->bootfail_logs[0].log_dir, BFM_DBG_FILE_NAME);
    snprintf(src_file_path, BFMR_MAX_PATH - 1, "%s/%s", BFM_QCOM_LOG_ROOT_PATH, BFM_DBG_FILE_NAME);
    ...
    bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir, BFM_DBG_FILE_NAME, pdata, src_file_len, 0);
    bfmr_free(pdata);
    oldfs = get_fs();
