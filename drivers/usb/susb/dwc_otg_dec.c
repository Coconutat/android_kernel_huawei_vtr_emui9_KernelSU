...
int dec_enable_abb_clk(struct otg_dev *otg_device)
{
...
void dec_disable_abb_clk(struct otg_dev *otg_device)
{
	void __iomem *pctrl_base = otg_device->pctrl_reg_base;
	uint32_t temp = 0;;
...
void init_usb_otg_phy_dec(struct otg_dev *otg_device, int is_host_mode)
{
	...
	if (is_host_mode) {
		writel(otg_device->host_eyePattern, &ahbif->usbotg2_ctrl3);
	} else {
...
void close_usb_otg_phy_dec(struct otg_dev *otg_device)
{
	void __iomem *pericrg_base = otg_device->pericrg_base;
...
static void dec_dplus_pulldown(struct otg_dev *otg_device)
{
	usb_dbg("+\n");
...
static void dec_dplus_pullup(struct otg_dev *otg_device)
{
	usb_dbg("+\n");
...
static void dec_check_voltage(struct otg_dev *otg_device)
{
	usb_dbg("+\n");

	if (otg_device->check_voltage) {
		/*first dplus pulldown*/
		dec_dplus_pulldown(otg_device);
		/*second call charger's API to check voltage */
		water_detect();
		/*third dplus pullup*/
		dec_dplus_pullup(otg_device);
	}

	usb_dbg("-\n");
}

struct usb_phy_ops dec_usb_phy_ops = {
	.init = init_usb_otg_phy_dec,
	.close = close_usb_otg_phy_dec,
	.enable_clk = dec_enable_abb_clk,
	.disable_clk = dec_disable_abb_clk,
	.check_voltage = dec_check_voltage,
};

static int dwc_otg_dec_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	register_usb_phy_ops(dev_p, &dec_usb_phy_ops);
...
static int dwc_otg_dec_remove(struct platform_device *pdev)
{
	int ret = 0;
...
#ifdef CONFIG_OF
static const struct of_device_id dwc_otg_dec_mach[] = {
	{ .compatible = "hisilicon,dec-usb-otg" },
	{},
};
MODULE_DEVICE_TABLE(of, dwc_otg_dec_mach);
#else
#define dwc_otg_dec_mach NULL
#endif

static struct platform_driver dwc_otg_dec_driver = {
	.probe		= dwc_otg_dec_probe,
	.remove		= dwc_otg_dec_remove,
	.driver		= {
		.name	= "dec_otg",
		.of_match_table = of_match_ptr(dwc_otg_dec_mach),
		.pm = dwc_otg_dev_pm_ops_p,
	},
};

module_platform_driver(dwc_otg_dec_driver);

MODULE_AUTHOR("z00289564");
MODULE_LICENSE("Dual BSD/GPL");
MODULE_DESCRIPTION("DesignWare OTG2.0 dec Glue Layer");
