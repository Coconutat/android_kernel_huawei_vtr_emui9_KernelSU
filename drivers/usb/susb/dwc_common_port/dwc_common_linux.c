...
{
	return memset(dest, byte, size);/* [false alarm]:this is original code */
}
...
{
	return strcmp(s1, s2);/* [false alarm]:this is original code */
}
...
#endif
	DWC_DEBUG("Work done: %s, container=%pK", container->name, container);
	if (container->name) {
