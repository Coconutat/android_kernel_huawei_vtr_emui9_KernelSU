	...
	u8 reg, u8 *data)
{
	...
	if (rc < 0) {
		...
		if (!dsm_client_ocuppy(client_pmic) && (dsm_notify_limit < 3)) {
			dsm_client_record(client_pmic, "pmic i2c read fail! reg=0x%x, rc=%d \n", reg, rc);
			dsm_client_notify(client_pmic, DSM_CAMPMIC_I2C_ERROR_NO);
	...
	u8 reg, u8 data)
{
	...
	if (rc < 0) {
		...
		if (!dsm_client_ocuppy(client_pmic) && (dsm_notify_limit < 3)) {
			dsm_client_record(client_pmic, "pmic i2c write fail! reg=0x%x, rc=%d \n", reg, rc);
			dsm_client_notify(client_pmic, DSM_CAMPMIC_I2C_ERROR_NO);
			cam_warn("[I/DSM] %s dsm_client_notify", client_pmic->client_name);
			dsm_notify_limit++;
		}
