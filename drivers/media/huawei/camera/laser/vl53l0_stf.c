...
int vl53l0_stf_get_dt_data(struct platform_device *pdev, hw_vl53l0_t *laser)
{
...
static int32_t vl53l0_platform_probe(struct platform_device *pdev)
{
	...
	//get dt data
	if (pdev->dev.of_node) {
		rc = vl53l0_stf_get_dt_data(pdev, laser);
		if (rc < 0) {
...
static const struct of_device_id hw_vl53l0_dt_match[] = {
	{.compatible = "huawei,vl53l0_stf"},
	{}
};

MODULE_DEVICE_TABLE(of, hw_vl53l0_dt_match);
static struct platform_driver  vl53l0_stf_platform_driver = {
	.driver = {
		.name = "vl53l0_stf",
		.owner = THIS_MODULE,
...
static int __init vl53l0_stf_mod_init(void)
{
	int rc = 0;
	cam_info(" Enter %s ", __func__);

	rc = platform_driver_probe(&vl53l0_stf_platform_driver, vl53l0_platform_probe);
	if (rc < 0) {
...
static void __exit vl53l0_stf_mod_exit(void)
{
...
module_init(vl53l0_stf_mod_init);
module_exit(vl53l0_stf_mod_exit);
