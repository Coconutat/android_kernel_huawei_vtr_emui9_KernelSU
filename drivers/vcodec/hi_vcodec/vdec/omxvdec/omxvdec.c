...
#define DBG_CMD_RESERVEDNUM     "set_ReservedNum"
#define DBG_CMD_SEGSIZE         "set_SegSize"
#define DBG_CMD_DFS             "set_Dfs"
#define DBG_CMD_POWER_ON        "set_PowerOn"
#define DBG_CMD_POWER_OFF       "set_PowerOff"
#define DBG_CMD_SUSPEND         "set_Suspend"
#define DBG_CMD_RESUME          "set_Resume"
...
/*================ GLOBAL VALUE ================*/
HI_U32   g_ExtraDispNum          = 3;
HI_U32   g_ClientReservedNum     = 4;         // Client may reserve some buffer, so we need to consider such expense.
HI_U32   g_SegSize               = 4;         // (M)
...
HI_VOID omxvdec_release_mem(MEM_BUFFER_S *pBuffer)
{
    if (HI_NULL != pBuffer)
    {
        ...
        memset(pBuffer, 0, sizeof(MEM_BUFFER_S)); /* unsafe_function_ignore: memset */
    }
    else
    {
...
static HI_S32 omxvdec_open(struct inode *inode, struct file *fd)
{
    ...
    printk(KERN_INFO "enter %s , open_count : %d\n", __func__, omxvdec->open_count);
    ...
    if (omxvdec->open_count < MAX_OPEN_COUNT)
    {
        ...
        {
           memset(&g_OmxFunc, 0, sizeof(OMXVDEC_FUNC)); /* unsafe_function_ignore: memset */

           ret = channel_init();
           if(ret != HI_SUCCESS)
           {
               printk(KERN_CRIT "%s init channel failed!\n", __func__);
               goto error0;
           }
           ...
           if (HI_TRUE == g_DynamicFsEnable && HI_NULL == omxvdec->task.task_thread)
           {
               ...
               if (ret != HI_SUCCESS)
               {
                   printk(KERN_CRIT "%s call channel_create_task failed!\n", __func__);
                   goto error1;
               }
           }
        }
...
static HI_S32 omxvdec_release(struct inode *inode, struct file *fd)
{
    ...
    if (HI_NULL == fd->private_data)
    {
        OmxPrint(OMX_FATAL, "%s: fd->private_data = null, error!\n", __func__);
        VDEC_UP_INTERRUPTIBLE(&omxvdec->omxvdec_mutex);
        return -EFAULT;
    }
...
static long omxvdec_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
    ...
    if (HI_NULL == u_arg)
    {
        OmxPrint(OMX_FATAL, "%s: param is null\n", __func__);
        return -EFAULT;
    }
    ...
    if (code != VDEC_IOCTL_CHAN_CREATE)
    {
        ...
        if (CHAN_STATE_INVALID == pchan->channel_state)
        {
            OmxPrint(OMX_ERR, "%s channel state is INVALID\n", __func__);
            return -EFAULT;
        }

        if (pchan->file_dec != (HI_U64)fd)
        {
            OmxPrint(OMX_ERR, "%s pchan's fd mismatch with caller's fd.\n", __func__);
            return -EFAULT;
        }
    }
    ...
    switch (code)
    {
        ...
        case VDEC_IOCTL_CHAN_ALLOC_BUFFER:
            ...
            {
                OmxPrint(OMX_FATAL, "%s %d: case call copy_from_user failed!\n", __func__, __LINE__);
                channel_free_user_buffer(pchan, &user_buf);
                return -EIO;
            }
            break;
...
static long omxvdec_compat_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
    ...
    switch (code)
    {
        ...
        case VDEC_IOCTL_CHAN_FREE_BUFFER:
            ...
            if (ret != HI_SUCCESS)
            {
                OmxPrint(OMX_INFO, "%s %d: case call free_buffer failed!\n", __func__, __LINE__);
                return -EFAULT;
            }
            break;
...
static HI_S32 omxvdec_probe(struct platform_device *pltdev)
{
    ...
    if(fpga_es == 1)
    {
        ...
        viraddr_pctrl  = (HI_U32 *)ioremap(PCTRL_PERI, 4);
        if (viraddr_pctrl)
        {
    ...
    else if(fpga_cs == 1)
    {
        ...
        return HI_FAILURE; //return for temp test
    }
    else
    {
    ...
    if (ret != HI_SUCCESS)
    {
        printk(KERN_CRIT "%s call channel probe failed!\n", __func__);
        goto cleanup0;
    }
...
HI_VOID omxvdec_help_proc(HI_VOID)
{
    ...
    "cmd = set_DispNum,      para = value        :set ExtraDispNum          = value\n"
    "cmd = set_ReservedNum,  para = value        :set ClientReservedNumNum  = value\n"
...
static HI_S32 omxvdec_read_proc(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "%-20s :%d\n", "OpneCount",         g_pOmxVdec->open_count);
    ...
    PROC_PRINT(p, "%-20s :%s\n", "SaveName",          g_SaveName);
    PROC_PRINT(p, "%-20s :%s\n", "SavePath",          g_SavePath);
    PROC_PRINT(p, "%-20s :%d\n", "DispNum",           g_ExtraDispNum);
    PROC_PRINT(p, "%-20s :%d\n", "ReservedNum",       g_ClientReservedNum);
    PROC_PRINT(p, "%-20s :%d\n", "SegSize(M)",        g_SegSize);
...
HI_S32 omxvdec_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    else
    {
        ...
        {
            if (j > 0 && j < (HI_S32)sizeof(g_SavePath) && str2[0] == '/')
            {
                if(str2[j-1] == '/')
                {
                   str2[j-1] = 0;
                }
                strncpy(g_SavePath, str2, PATH_LEN);/* unsafe_function_ignore: strncpy  */
        ...
        {
            if (j > 0 && j < (HI_S32)sizeof(g_SaveName))
            {
                strncpy(g_SaveName, str2, NAME_LEN);/* unsafe_function_ignore: strncpy  */
        ...
        // cppcheck-suppress *
        else if (!strncmp(str1, DBG_CMD_RESERVEDNUM, DBG_CMD_LEN))
        {
            if(omxvdec_string_to_value(str2, &dat2) != 0)
            {
                goto error;
            ...
            OmxPrint(OMX_ALWS, "ClientReservedNum: %u\n", dat2);
            g_ClientReservedNum = dat2;
        }
        // cppcheck-suppress *
        ...
        // cppcheck-suppress *
        else if (!strncmp(str1, DBG_CMD_POWER_ON, DBG_CMD_LEN))
        {
            OmxPrint(OMX_ALWS, "Set regulator enable.\n");
            VDEC_Regulator_Enable();
        }
        // cppcheck-suppress *
        else if (!strncmp(str1, DBG_CMD_POWER_OFF, DBG_CMD_LEN))
        {
            OmxPrint(OMX_ALWS, "Set regulator disable.\n");
            VDEC_Regulator_Disable();
        }
        // cppcheck-suppress *
        else if (!strncmp(str1, DBG_CMD_SUSPEND, DBG_CMD_LEN))
        {
            OmxPrint(OMX_ALWS, "Test suspend.\n");
            channel_suspend();
        }
        // cppcheck-suppress *
        else if (!strncmp(str1, DBG_CMD_RESUME, DBG_CMD_LEN))
        {
            OmxPrint(OMX_ALWS, "Test resume.\n");
            channel_resume();
        }
        // cppcheck-suppress *
...
HI_S32 omxvdec_init_proc (HI_VOID)
{
    ...
    /* Create proc */
    snprintf(aszBuf, sizeof(aszBuf), "omxvdec");/* unsafe_function_ignore: snprintf  */
