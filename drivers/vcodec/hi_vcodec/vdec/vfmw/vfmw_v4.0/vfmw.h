...
typedef struct
{
    ...
    SINT32           s32PathMode;          /* for CSD PATHMODE */
    UINT32           s32FrameRate;         /* for CSD FrameRate */
    UINT32           s32VideoScenario;     /* video Vilte Mode add for CSD */
...
typedef struct
{
    MASTERING_DISPLAY_COLOUR_VOLUME_S mastering_info;
    CONTENT_LIGHT_LEVEL_INFO_S content_info;
    COLOUR_DESCRIPTION_INFO_S colour_info;
} VDEC_HDR_S;
...
typedef struct
{
    ...
    UINT32  is_cur_last;  // current last frame in queue
    ...
    UINT64  Usertag;
    UINT64  DispTime;
    VDEC_USRDAT_S*  p_usrdat[4];   // NOTICE!!! in tvp path, these member invalid for 32 & 64 system difference
#ifdef ENV_SOS_KERNEL 
...
} VDEC_CHAN_FRAME_NODE;

/*For dynamic seperated frame store param */
typedef struct
{
	UINT32                TotalFrameNum;
    VDEC_CHAN_FRAME_NODE  Node[MAX_FRAME_NUM];
...
typedef struct
{
    ...
    UINT32 BpdIrqNum;
    UINT32 SmmuIrqNum;
    UINT32 MfdeSafeIrqNum;
    UINT32 ScdSafeIrqNum;
    UINT32 BpdSafeIrqNum;
    UINT32 SmmuSafeIrqNum;
...
typedef SINT32 (*EXT_FN_MEM_MALLOC)(SINT8*, UINT32, UINT32, UINT32, VOID*);
typedef VOID   (*EXT_FN_MEM_FREE)(VOID*);
typedef SINT32 (*EXT_FN_POWER_ON)(VOID);
typedef SINT32 (*EXT_FN_POWER_OFF)(VOID);
typedef SINT32 (*EXT_FN_GET_CLK_RATE)(CLK_RATE_E*);
typedef SINT32 (*EXT_FN_SET_CLK_RATE)(CLK_RATE_E);
...
typedef struct
{
    ...
    EXT_FN_GET_CLK_RATE    get_clk_rate;
    EXT_FN_SET_CLK_RATE    set_clk_rate;
} INIT_INTF_S;
...
SINT32 VDEC_Suspend(UINT8 IsSecure);
SINT32 VDEC_Resume(UINT8 IsSecure);
