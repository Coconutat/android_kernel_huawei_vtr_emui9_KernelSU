...
#define DTS_COMP_JDI_NT35695 "hisilicon,external_mipi_jdi_NT35695"
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (g_lcd_fpga_flag == 0) {
			...
			mipi_dsi_cmds_tx(lcd_display_init_cmds, \
				ARRAY_SIZE(lcd_display_init_cmds), mipi_dsi1_base);

			mipi_dsi_cmds_tx(lcd_bl_level_adjust, \
				ARRAY_SIZE(lcd_bl_level_adjust), mipi_dsi1_base);

			if (is_mipi_cmd_panel(hisifd)) {
				mipi_dsi_cmds_tx(lcd_display_command_cmds, \
					ARRAY_SIZE(lcd_display_command_cmds), mipi_dsi1_base);
			} else {
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "jdi_NT35695 5.2' EXTERNAL  TFT 1080 x 1920\n");
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	if (g_lcd_fpga_flag == 0) {
		// GPIO_EP0
		ret = of_property_read_u32_index(np, "gpio_nums", 0, &gpio_lcd_p5v5_enable);
		if (ret != 0) {
			HISI_FB_ERR("failed to get gpio_lcd_p5v5_enable resource! ret=%d.\n", ret);
			gpio_lcd_p5v5_enable = 256;
		}
		//GPIO_EP1
		ret = of_property_read_u32_index(np, "gpio_nums", 1, &gpio_lcd_n5v5_enable);
		if (ret != 0) {
			HISI_FB_ERR("failed to get gpio_lcd_n5v5_enable resource! ret=%d.\n", ret);
			gpio_lcd_n5v5_enable = 257;
		}
		//GPIO_EP4
		ret = of_property_read_u32_index(np, "gpio_nums", 2, &gpio_lcd_bl_enable);
		if (ret != 0) {
			HISI_FB_ERR("failed to get gpio_lcd_bl_enable resource! ret=%d.\n", ret);
			gpio_lcd_bl_enable = 260;
		}
		//GPIO_EP6
		ret = of_property_read_u32_index(np, "gpio_nums", 3, &gpio_lcd_reset);
		if (ret != 0) {
			HISI_FB_ERR("failed to get gpio_lcd_reset resource! ret=%d.\n", ret);
			gpio_lcd_reset = 262;
		}
	}
	HISI_FB_INFO("gpio_lcd_p5v5_enable = %d, gpio_lcd_n5v5_enable = %d, gpio_lcd_bl_enable = %d, gpio_lcd_reset = %d\n",
		gpio_lcd_p5v5_enable, gpio_lcd_n5v5_enable, gpio_lcd_bl_enable, gpio_lcd_reset);
...
static struct platform_driver this_driver = {
	...
	.shutdown = NULL,
	.driver = {
		.name = "external_mipi_jdi_NT35695",
		.owner = THIS_MODULE,
