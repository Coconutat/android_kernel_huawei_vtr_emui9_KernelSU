...
#include "mipi_jdi_NT35696_5p5.h"
#include <huawei_platform/log/log_jank.h>
#include <linux/hisi/hw_cmdline_parse.h>

#define NT35696_30FPS 0

#define DTS_COMP_JDI_NT35696_5p5 "hisilicon,mipi_jdi_NT35696_5p5"
#define CABC_OFF	(0)
...
#define CHECKSUM_PIC_N  (3)

static int checksum_pic_n = 0;
static uint32_t expected_checksum1[CHECKSUM_PIC_N][CHECKSUM_SIZE] = {
	{0x31, 0xec, 0x6b, 0x4a, 0x1d, 0x60, 0xdd, 0x51}, //runningtest1
	{0x29, 0x46, 0x28, 0x67, 0x26, 0x2e, 0x6d, 0x3f}, //runningtest2
	{0xc5, 0x2a, 0x53, 0x8a, 0x94, 0x94, 0x9, 0xf9}, //runningtest3
};

static uint32_t expected_checksum2[CHECKSUM_PIC_N][CHECKSUM_SIZE] = {
	{0xc1, 0xb4, 0xb3, 0xd5, 0x9e, 0x34, 0x23, 0x93}, //runningtest1
	{0xd8, 0x7d, 0xe8, 0xe, 0x70, 0x72, 0xa1, 0xeb}, //runningtest2
	{0xa, 0xe3, 0x7, 0x5d, 0x10, 0x64, 0x2e, 0x37}, //runningtest3
};
...
#define JDI_NT35696_OK 0

static struct jdi_nt35696_device g_dev_info;
...
//static char novatek_cmd2[] = { 0xc4,0x01 };
...
//static char novatek_cmd22[] = { 0xd4,0x07 };
//static char novatek_cmd23[] = { 0xd5,0x8a };
...
static char CMD2_page4_cmd1[] = {
	0xf3,
	0x06,
};

static char novatek_cmd2_fflash[] = { 0xff,0x10 };
#if NT35696_30FPS
static struct dsi_cmd_desc nt35696_fps_to_30[] = {
...
static struct dsi_cmd_desc nt35696_fps_to_60[] = {
...
static struct dsi_cmd_desc lcd_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(no_laod), no_laod},
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(CMD2_page4_cmd1), CMD2_page4_cmd1},
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(novatek_cmd2_fflash), novatek_cmd2_fflash},
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "JDI_NT35696 5'5 CMD TFT\n");
...
static int mipi_jdi_panel_lcd_lv_detect(struct platform_device *pdev)
{
	struct hisi_fb_data_type *hisifd = NULL;
	char __iomem *mipi_dsi0_base = NULL;
	uint32_t rd = 0;
	...
	char cmd2_lv_detect_enable[] = {0x5d, 0x02};
	char cmd2_lv_detect_disable[] = {0x5d, 0x00};
	char lv_detect_read[] = {0x52};
	...
	struct dsi_cmd_desc lv_detect_read_reg[] = {
	...
	mipi_dsi_cmds_rx(&rd, lv_detect_read_reg, \
		ARRAY_SIZE(lv_detect_read_reg), mipi_dsi0_base);
	HISI_FB_INFO("rd : %02x\n", rd);
	if (rd) {
		HISI_FB_ERR("rd = 0x%x, expected = 0x00\n", rd);
		lv_detect_result = 1;
	}else{
		HISI_FB_INFO("no vsp/vsn voltage error\n");
		lv_detect_result = 0;
	}
...
static int mipi_jdi_panel_lcd_current_detect(struct platform_device *pdev)
{
	struct hisi_fb_data_type *hisifd = NULL;
	char __iomem *mipi_dsi0_base = NULL;
	uint32_t rd = 0;
	char cmd2_page0_select[] = {0xFF, 0x20};
	char cmd2_no_load[] = {0xFB, 0x01};
	char cmd2_current_check_enable[] = {0x63, 0xF3};
	char cmd2_current_check_disable[] = {0x63, 0x00};
	char current_check_read[] = {0x51};
	char cmd1_page_select[] = {0xFF, 0x10};
	...
	struct dsi_cmd_desc lcd_current_check_enable_cmds[] = {
		{DTYPE_DCS_WRITE1, 0, 100, WAIT_TYPE_US,
			sizeof(cmd2_current_check_enable), cmd2_current_check_enable},
	};

	struct dsi_cmd_desc lcd_current_check_disable_cmds[] = {
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(cmd2_current_check_disable), cmd2_current_check_disable},
	};
	...
	mipi_dsi_cmds_tx(lcd_no_load_cmds, \
		ARRAY_SIZE(lcd_no_load_cmds), mipi_dsi0_base);
	mipi_dsi_cmds_tx(lcd_current_check_enable_cmds, \
		ARRAY_SIZE(lcd_current_check_enable_cmds), mipi_dsi0_base);

	mipi_dsi_cmds_rx(&rd, lcd_current_check_reg, \
	...
	mipi_dsi_cmds_tx(lcd_current_check_disable_cmds, \
		ARRAY_SIZE(lcd_current_check_disable_cmds), mipi_dsi0_base);
	mipi_dsi_cmds_tx(lcd_cmd1_select_cmds, \
		ARRAY_SIZE(lcd_cmd1_select_cmds), mipi_dsi0_base);
...
#define	NT35696_EBABLE_CHECKSUM	1
#define	NT35696_DISABLE_CHECKSUM 0
#define	NT35696_TEST_PIC_0	3
#define	NT35696_TEST_PIC_1	4
#define	NT35696_TEST_PIC_2	5
#define	NT35696_PIC0_NUM	0
#define	NT35696_PIC1_NUM	1
#define	NT35696_PIC2_NUM	2
static ssize_t mipi_jdi_panel_lcd_gram_check_show(struct platform_device *pdev, char *buf)
{
	ssize_t ret = 0;
	struct hisi_fb_data_type *hisifd = NULL;
	char __iomem *mipi_dsi0_base = NULL;
	uint32_t rd1[CHECKSUM_SIZE] = {0};
	uint32_t rd2[CHECKSUM_SIZE] = {0};
	int i = 0;
	...
	HISI_FB_INFO("%s +\n", __func__);
	switch (checksum_pic_n){
		case NT35696_PIC0_NUM:
		case NT35696_PIC1_NUM:
		case NT35696_PIC2_NUM:
			HISI_FB_INFO("%s start check gram checksum:[%d]\n", __func__, checksum_pic_n);
			break;
		default:
	...
	for (i = 0; i < CHECKSUM_SIZE; i++) {
		...
		*data = 0x73 + i;
		...
		if (rd1[i] != expected_checksum1[checksum_pic_n][i]) {
			HISI_FB_ERR("rd[%d] = 0x%x, pic_num=%d, expected = 0x%x\n", i, rd1[i], checksum_pic_n,
				expected_checksum1[checksum_pic_n][i]);
			checksum_result++;
		}
	}
	HISI_FB_INFO("0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x, pic_num=%d\n", \
			rd1[0], rd1[1], rd1[2], rd1[3], rd1[4], rd1[5], rd1[6], rd1[7], checksum_pic_n);
	...
	for (i = 0; i < CHECKSUM_SIZE; i++) {
		...
		*data = 0x73 + i;
		...
		if (rd2[i] != expected_checksum2[checksum_pic_n][i]) {
			HISI_FB_ERR("rd[%d] = 0x%x, pic_num=%d, expected = 0x%x\n", i, rd2[i], checksum_pic_n,
				expected_checksum2[checksum_pic_n][i]);
			checksum_result++;
		}
	}
	HISI_FB_INFO("0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x, pic_num=%d\n", \
			rd2[0], rd2[1], rd2[2], rd2[3], rd2[4], rd2[5], rd2[6], rd2[7], checksum_pic_n);
	...
	const char *buf, size_t count)
{
	...
	/* gram checksum enable */
	char cmd3_enable[] = {0xFF, 0xf0};
	char checksum_clk_enable[] = {0x92, 0x01};
	/* gram checksum disable */
	char checksum_clk_disable[] = {0x92, 0x00};
	char checksum_page_select[] = {0x7B, 0x00};
	...
	struct dsi_cmd_desc lcd_checksum_enable_cmds[] = {
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(cmd3_enable), cmd3_enable},
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(checksum_clk_enable), checksum_clk_enable},
	};
	...
	struct dsi_cmd_desc lcd_checksum_disable_cmds[] = {
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(cmd3_enable), cmd3_enable},
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(checksum_clk_disable), checksum_clk_disable},
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
			sizeof(checksum_page_select), checksum_page_select},
		{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
	...
	switch(val){
		...
		case NT35696_EBABLE_CHECKSUM:
			...
			break;
		case NT35696_TEST_PIC_0:
		case NT35696_TEST_PIC_1:
		case NT35696_TEST_PIC_2:
			HISI_FB_INFO("gram checksum set pic [%d]\n", val);
			checksum_pic_n = val - 3;
			break;
		default:
			HISI_FB_INFO("gram checksum set pic [%d] unknown\n", val);
			checksum_pic_n = -1;
			break;
	}
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "checksum:1;bl_open_short:1\n");
...
static struct dsi_cmd_desc nt35696_disable_dfr[] = {
	{DTYPE_DCS_WRITE1, 0, 200, WAIT_TYPE_US,
...
static struct dsi_cmd_desc nt35696_enable_dfr[] = {
	{DTYPE_DCS_WRITE1, 0, 200, WAIT_TYPE_US,
		sizeof(novatek_cmd2_open), novatek_cmd2_open},
	{DTYPE_DCS_WRITE1, 0, 200, WAIT_TYPE_US,
...
static int mipi_jdi_panel_lcd_fps_scence_handle(struct platform_device *pdev, uint32_t scence)
{
	...
	switch(scence){
		...
		case LCD_FPS_SCENCE_VIDEO:
			...
			mipi_dsi_cmds_tx(nt35696_disable_dfr,
					ARRAY_SIZE(nt35696_disable_dfr), mipi_dsi0_base);
		...
		case LCD_FPS_SCENCE_GAME:
			...
			hisifb_set_vsync_activate_state(hisifd, true);
			hisifb_activate_vsync(hisifd);
			mipi_dsi_cmds_tx(nt35696_enable_dfr,
					ARRAY_SIZE(nt35696_enable_dfr), mipi_dsi0_base);
			hisifb_set_vsync_activate_state(hisifd, false);
			hisifb_deactivate_vsync(hisifd);
			break;
...
static int mipi_jdi_panel_lcd_fps_updt_handle(struct platform_device *pdev){
	...
	switch (pinfo->fps_updt){
		case LCD_FPS_30:
			...
			hisifb_set_vsync_activate_state(hisifd, true);
			hisifb_activate_vsync(hisifd);
			mipi_dsi_cmds_tx(nt35696_fps_to_30,
					ARRAY_SIZE(nt35696_fps_to_30), mipi_dsi0_base);
			hisifb_set_vsync_activate_state(hisifd, false);
			hisifb_deactivate_vsync(hisifd);
			break;
		case LCD_FPS_60:
			...
			hisifb_set_vsync_activate_state(hisifd, true);
			hisifb_activate_vsync(hisifd);
			mipi_dsi_cmds_tx(nt35696_fps_to_60,
					ARRAY_SIZE(nt35696_fps_to_60), mipi_dsi0_base);
			hisifb_set_vsync_activate_state(hisifd, false);
			hisifb_deactivate_vsync(hisifd);
			break;
		default:
...
static int mipi_jdi_get_dts_config(void){
	...
	HISI_FB_INFO("%s +\n", __func__);
	memset(&g_dev_info, 0, sizeof(struct jdi_nt35696_device));

	np = of_find_compatible_node(NULL, NULL, DTS_COMP_JDI_NT35696_5p5);
	if (!np) {
		HISI_FB_ERR("not found device node %s!\n", DTS_COMP_JDI_NT35696_5p5);
		return JDI_NT35696_ERR;
	}

	//"lcd-display-type"
	ret = of_property_read_u32(np, LCD_DISPLAY_TYPE_NAME, &g_dev_info.display_type);
	if (ret) {
		HISI_FB_ERR("get lcd_display_type failed!\n");
		g_dev_info.display_type = PANEL_MIPI_CMD;
	}
	...
	HISI_FB_INFO("%s: g_dev_info.display_type:%u\n", __func__, g_dev_info.display_type);
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	pinfo->type = g_dev_info.display_type;
...
static const struct of_device_id hisi_panel_match_table[] = {
	{
		.compatible = DTS_COMP_JDI_NT35696_5p5,
		.data = NULL,
	},
	{},
};
