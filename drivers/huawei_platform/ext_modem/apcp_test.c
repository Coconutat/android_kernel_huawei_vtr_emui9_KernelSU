...
static struct dsm_client *gp_modem_dsm_client;
...
void apcp_current_subpacket_printk(char *pt_buf, unsigned int t_count,
				   unsigned char t_is_tx,
				   unsigned int t_chan_num)
{
	...
	if (t_is_tx == 1) {
		g_apcp_test.m_print_buff_pos +=
		    snprintf(&g_apcp_test.m_print_buff
			     [g_apcp_test.m_print_buff_pos],
			     (APCP_PRINT_BUFFER_SIZE -
			      g_apcp_test.m_print_buff_pos), "write %d to ",
			     count);
	} else {
		g_apcp_test.m_print_buff_pos +=
		    snprintf(&g_apcp_test.m_print_buff
			     [g_apcp_test.m_print_buff_pos],
			     (APCP_PRINT_BUFFER_SIZE -
			      g_apcp_test.m_print_buff_pos), "read %d from ",
			     count);
	}

	g_apcp_test.m_print_buff_pos +=
	    snprintf(&g_apcp_test.m_print_buff[g_apcp_test.m_print_buff_pos],
		     (APCP_PRINT_BUFFER_SIZE - g_apcp_test.m_print_buff_pos),
	...
	g_apcp_test.m_print_buff_pos +=
	    snprintf(&g_apcp_test.m_print_buff[g_apcp_test.m_print_buff_pos],
		     (APCP_PRINT_BUFFER_SIZE - g_apcp_test.m_print_buff_pos),
		     "rx irq=%d,cpwkap=%d", g_apcp_test.m_rx_irq_cnt,
	...
	for (i = 0; i < (MODEM_CHANNEL_NR - 1); i++) {
		g_apcp_test.m_print_buff_pos +=
		    snprintf(&g_apcp_test.m_print_buff
			     [g_apcp_test.m_print_buff_pos],
			     (APCP_PRINT_BUFFER_SIZE -
			      g_apcp_test.m_print_buff_pos), "%x/%x,",
			     g_apcp_test.m_txpkts_on_chans[i],
			     g_apcp_test.m_rxpkts_on_chans[i]);
	}
	...
	g_apcp_test.m_print_buff_pos +=
	    snprintf(&g_apcp_test.m_print_buff[g_apcp_test.m_print_buff_pos],
		     (APCP_PRINT_BUFFER_SIZE - g_apcp_test.m_print_buff_pos),
		     "%x/%x]\n",
		     g_apcp_test.m_txpkts_on_chans[MODEM_CHANNEL_NR - 1],
		     g_apcp_test.m_rxpkts_on_chans[MODEM_CHANNEL_NR - 1]);

	g_apcp_test.m_print_buff_pos +=
	    snprintf(&g_apcp_test.m_print_buff[g_apcp_test.m_print_buff_pos],
		     (APCP_PRINT_BUFFER_SIZE - g_apcp_test.m_print_buff_pos),
		     "%x/%x\n",
		     g_apcp_test.m_txpkts_on_chans[MODEM_CHANNEL_FLASHLESS_NUM],
		     g_apcp_test.m_rxpkts_on_chans
		     [MODEM_CHANNEL_FLASHLESS_NUM]);
...
void apcp_txrx_buffer_count(unsigned int t_chan_num, unsigned int t_count,
			    unsigned char t_is_tx)
{
	if (t_is_tx) {
