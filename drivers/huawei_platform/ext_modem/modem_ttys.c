...
static int check_tty_permission(struct modem_tty_dev *pt_tty_dev)
{
	...
	if ((pt_tty_dev->m_tty_index != 0)
		...
		hwlog_err
		    ("%s %d: Flashlessd is downloading cp.rom and fsm images, pt_tty_dev m_tty_index is %d.\n",
...
static void read_buffered_data_before_open(struct modem_tty_dev *pt_tty_dev)
{
	...
	if (p_tty) {
		...
		while (!list_empty(&pt_tty_dev->m_buf_in_list)) {
			p_packet =
			    list_first_entry(&pt_tty_dev->m_buf_in_list,
			...
			if (p_packet) {
				room =
				    ...
				    (&pt_tty_dev->m_tty_port, p_packet->m_size);

				if (room < p_packet->m_size) {
					hwlog_debug
					    ("%s %d: port %d no room in tty rx buffer!\n",
					     __func__, __LINE__,
					     pt_tty_dev->m_tty_index);
				} else {
					room =
					    tty_insert_flip_string
					    (&pt_tty_dev->m_tty_port,
					     p_packet->pm_buffer,
					     p_packet->m_size);

					if (room < p_packet->m_size) {
						hwlog_err
						    ("%s %d: port %d couldn't insert all characters (TTY is full?)!\n",
						     __func__, __LINE__,
						     pt_tty_dev->m_tty_index);
					} else {
...
static void clean_buffered_data_on_close(struct modem_tty_dev *pt_tty_dev)
{
	...
	while (!list_empty(&pt_tty_dev->m_buf_in_list)) {
		p_packet =
		    list_first_entry(&pt_tty_dev->m_buf_in_list,
				     struct modem_buf_in_packet, m_node);
...
static int modem_tty_install(struct tty_driver *pt_driver,
			     struct tty_struct *pt_tty)
{
	...
	if (ret) {
		hwlog_err
		    ("%s %d: tty dev %d tty_init_termios return error, ret=%d.\n",
...
static int modem_tty_write_room(struct tty_struct *pt_tty)
{
	...
	if (ret < 0) {
		...
		space_len =
		    FIFO_SIZE -
		    get_channel_kfifo_data_length(p_tty_dev->m_tty_index + 1);
	}
...
static void modem_tty_set_termios(struct tty_struct *pt_tty,
...
static int modem_tty_tiocmset(struct tty_struct *pt_tty, unsigned int set,
			      unsigned int clear)
{
	struct modem_tty_dev *p_tty_dev = pt_tty->driver_data;
	int ret;
 ...
 */
static int modem_tty_write(struct tty_struct *pt_tty,
			   const unsigned char *pt_buf, int t_count)
{
	struct modem_tty_dev *p_tty_dev = pt_tty->driver_data;
	int ret = 0;
	...
	if ((NULL == pt_tty) || (NULL == pt_buf) || (0 == t_count)) {
		hwlog_err
		    ("%s %d: NULL pointer! pt_tty=0x%lx, pt_buf=0x%lx, count=%d\n",
		     __func__, __LINE__, (unsigned long)pt_tty,
		     (unsigned long)pt_buf, t_count);
		return -1;
	}

	hwlog_debug("%s %d: tty index is %d, write length=%d\n", __func__,
		    __LINE__, p_tty_dev->m_tty_index, t_count);
	ret = check_tty_permission(p_tty_dev);
	...
	ret =
	    modem_tty_write_to_channel(p_tty_dev->m_tty_index, pt_buf, t_count);
...
static int modem_tty_port_activate(struct tty_port *pt_tty_port,
				   struct tty_struct *pt_tty)
{
	struct modem_tty_dev *p_tty_dev = NULL;
...
static void modem_tty_remove(struct ext_modem *modem)
{
	...
	for (index = 0; index < MODEM_TTY_NR; index++) {
		...
		if (p_tty) {
			hwlog_info
			    ("%s %d destory tty,index=%d tty device index=%d\n",
 ...
 */
int modem_tty_push(int t_tty_index, unsigned char *pt_payload_buf,
		   int t_payload_len)
{
	struct modem_tty_dev *p_tty_dev = NULL;
	struct tty_struct *p_tty = NULL;
	...
	if ((NULL == pt_payload_buf) || (0 == t_payload_len)) {
		hwlog_err
		    ("%s %d: input payload buffer is 0x%lx, length is %d\n",
		     __func__, __LINE__, (unsigned long)pt_payload_buf,
		     t_payload_len);
		return -1;
	}
	...
	if (NULL == p_tty_dev) {
		hwlog_err
		    ("%s %d p_tty_dev==NULL on channel index t_tty_index:%d\n",
	...
	if (!p_tty) {
		...
		if (p_tty_dev->m_buf_in_size > MODEM_BUF_IN_MAX_SIZE) {
			...
			packet =
			    kzalloc(sizeof(struct modem_buf_in_packet),
				    GFP_KERNEL);
			if (!packet) {
				hwlog_err
				    ("%s %d: dev %d kzalloc packet error\n",
				     __func__, __LINE__, t_tty_index);
				ret = -1;
				goto out;
			}
			...
			if (!packet->pm_buffer) {
				hwlog_err
				    ("%s %d: tty dev %d kzalloc packet buffer error\n",
				     __func__, __LINE__, t_tty_index);
				kfree(packet);
				ret = -2;
				goto out;
			...
			memcpy(packet->pm_buffer, pt_payload_buf,
			       packet->m_size);

			if (p_tty_dev->m_buf_in_num <
			    p_tty_dev->m_buf_in_maxnum) {
				list_add_tail(&packet->m_node,
					      &p_tty_dev->m_buf_in_list);
				p_tty_dev->m_buf_in_num++;
			} else {
				struct modem_buf_in_packet *old_packet = NULL;
				old_packet =
				    list_first_entry(&p_tty_dev->m_buf_in_list,
						     struct modem_buf_in_packet,
						     m_node);

				if (!old_packet) {
					hwlog_err
					    ("%s %d: the  packet cannot be found\n",
					     __func__, __LINE__);
	...
	if (p_tty && t_payload_len) {
		ret =
		    tty_buffer_request_room(&p_tty_dev->m_tty_port,
					    t_payload_len);

		if (ret < t_payload_len) {
			hwlog_debug
			    ("%s %d: ttyVIA%d no room in tty rx buffer!\n",
			     __func__, __LINE__, t_tty_index);
		} else {
			ret =
			    tty_insert_flip_string(&p_tty_dev->m_tty_port,
						   pt_payload_buf,
						   t_payload_len);

			if (ret < t_payload_len) {
				hwlog_err
				    ("%s %d: ttyVIA%d couldn't insert all characters (TTY is full?)!\n",
				     __func__, __LINE__, t_tty_index);
			} else {
...
int modem_ttys_init(void)
{
	...
	p_tty_drv->init_termios.c_lflag &=
	    ~(ICANON | ECHO | ECHOE | ECHOK | ECHONL | ISIG);
	p_tty_drv->init_termios.c_iflag &=
	...
	for (index = 0; index < MODEM_TTY_NR; index++) {
		...
		dev =
		    tty_port_register_device(&p_tty_dev->m_tty_port,
					     g_modem_ttys.pm_tty_drv,
