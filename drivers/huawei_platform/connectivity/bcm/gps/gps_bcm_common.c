...
static int __init gps_bcm_common_init(void)
{
	...
	if (!isMyConnectivityChip(CHIP_TYPE_BCM)) {
		pr_info("gps chip type is not bcm, skip to register_gps_set_ref_clk_func!");
		return 0;
	}
#endif
	...
	if (!np) {
		pr_err("%s, can't find node %s\n", __func__,
		       DTS_COMP_GPS_POWER_NAME);
		return -1;
	}

	ret =
	    of_property_read_string(np, "broadcom_config,ic_type",
				    (const char **)&gps_ic_type_str);
	if (ret) {
		pr_err("get broadcom_config,ic_type fail ret=%d\n",
		       ret);
		gps_ic_type = GPS_IC_TYPE_4752;
	} else if (0 ==
		   strncmp(STR_BCM_TYPE_4752, gps_ic_type_str,
			   sizeof(STR_BCM_TYPE_4752))) {
		gps_ic_type = GPS_IC_TYPE_4752;
	} else if (0 ==
		   strncmp(STR_BCM_TYPE_47531, gps_ic_type_str,
			   sizeof(STR_BCM_TYPE_47531))) {
	...
	pr_err("[%s]  gps_ic_type: %d\n", __func__, gps_ic_type);

	ret =
	    of_property_read_string(np, "broadcom_config,ref_clk_sel_enable",
				    (const char **)&gps_ref_clk_enable);
	if (ret) {
	...
	if (GPS_REFCLK_SEL_ENABLE == gps_ref_sel_enable) {
		register_gps_set_ref_clk_func((void *)
					      set_gps_ref_clk_enable_bcm);
	}
