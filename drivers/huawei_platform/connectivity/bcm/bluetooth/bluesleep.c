...
static struct bluetooth_sleep_data *g_bs_data;
...
static inline int bluesleep_can_sleep(void)
{
	...
	return (LPM_BT_WAKE_DEASSERT ==
		gpio_get_value(g_bs_data->gpio_host_wake_bt.gpio))
	    && (LPM_HOST_WAKE_DEASSERT ==
		gpio_get_value(g_bs_data->gpio_bt_wake_host.gpio));
...
{
	struct bluetooth_sleep_data *bs_data =
	    (struct bluetooth_sleep_data *)data;
	...
	if (LPM_HOST_WAKE_ASSERT ==
	    gpio_get_value(bs_data->gpio_bt_wake_host.gpio)) {
...
static void bluesleep_outgoing_data(struct bluetooth_sleep_data *bs_data)
{
	...
	if (LPM_BT_WAKE_DEASSERT ==
	    gpio_get_value(bs_data->gpio_host_wake_bt.gpio)) {
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	if (!test_bit(BT_TXDATA, &bs_data->flags)) {
		...
		gpio_set_value(bs_data->gpio_host_wake_bt.gpio,
			       LPM_BT_WAKE_DEASSERT);
		bluesleep_tx_idle();
	} else {
		pr_info("Tx data during last	period\n");
		mod_timer(&bs_data->tx_timer,
			  jiffies + (TX_TIMER_INTERVAL * HZ));
	}
...
static irqreturn_t bluesleep_hostwake_isr(int irq, void *dev_id)
{
	 ...
	 */
	struct bluetooth_sleep_data *bs_data =
	    (struct bluetooth_sleep_data *)dev_id;
...
static int bluesleep_start(struct bluetooth_sleep_data *bs_data)
{
	...
	if (retval < 0) {
		pr_err
		    ("%s: couldn't enable	BT_HOST_WAKE as	wakeup interrupt %d",
		     __func__, retval);
		goto err_en_irqwake;
	}
 ...
 */
static ssize_t bluesleep_ap_wakeup_bt(struct file *filp,
 ...
 */
static ssize_t bluepower_write_proc_btwake(struct file *filp,
					   const char __user *buffer,
					   size_t len, loff_t *off)
{
	char btwake;
...
static int create_bluesleep_proc_sys(struct bluetooth_sleep_data *bs_data)
{
	...
	if (bs_data->bluetooth_dir == NULL) {
		pr_err("%s: unable to create /proc/bluetooth directory",
		       __func__);
		retval = -ENOMEM;
		return retval;
	}
	...
	proc_create("proto", S_IWUGO | S_IFREG, bs_data->sleep_dir,
		    &bt_proc_fops_proto);

	/* write entries btwrite */
	proc_create("btwrite", S_IWUGO | S_IFREG, bs_data->sleep_dir,
		    &bt_proc_fops_btwrite);

	/* write entries btwake */
	proc_create("btwake", S_IWUGO | S_IFREG, bs_data->sleep_dir,
		    &bt_proc_fops_btwake);
...
static int bluesleep_probe(struct platform_device *pdev)
{
	...
	bluesleep_data =
	    devm_kzalloc(bluetooth_sleep_dev,
			 sizeof(struct bluetooth_sleep_data), GFP_KERNEL);
	if (bluesleep_data == NULL) {
		dev_err(bluetooth_sleep_dev,
			"failed to allocate bluetooth_sleep_data\n");
		return -ENOMEM;
	}
	...
	of_property_read_u32(np, "huawei,bt_wake_host",
			     &(bluesleep_data->gpio_bt_wake_host.gpio));
	of_property_read_u32(np, "huawei,host_wake_bt",
			     &(bluesleep_data->gpio_host_wake_bt.gpio));
#else
#if defined(CONFIG_BCM4343)
	bluesleep_data->gpio_bt_wake_host.gpio =
	    of_get_gpio_by_prop(np, "huawei,bt_wake_host", 0, 0, &gpio_flags);
#else
	of_property_read_u32(np, "huawei,bt_wake_host",
			     &(bluesleep_data->gpio_bt_wake_host.gpio));
#endif
...
#if defined(CONFIG_BCM4343)
	bluesleep_data->gpio_host_wake_bt.gpio =
	    of_get_gpio_by_prop(np, "huawei,host_wake_bt", 0, 0, &gpio_flags);
#else
	of_property_read_u32(np, "huawei,host_wake_bt",
			     &(bluesleep_data->gpio_host_wake_bt.gpio));
#endif
	...
	ret =
	    gpio_request(bluesleep_data->gpio_bt_wake_host.gpio,
			 "bt_wake_host");
	if (ret < 0) {
		pr_err("%s: gpio_request %d  failed, ret:%d .\n", __func__,
		       bluesleep_data->gpio_bt_wake_host.gpio, ret);
		goto free_g_bs_data;
	}

	ret =
	    gpio_request(bluesleep_data->gpio_host_wake_bt.gpio,
			 "host_wake_bt");
	if (ret < 0) {
		pr_err("%s: gpio_request %d  failed, ret:%d .\n", __func__,
		       bluesleep_data->gpio_host_wake_bt.gpio, ret);
	...
	/* deassert bt wake */
	ret =
	    gpio_direction_output(bluesleep_data->gpio_host_wake_bt.gpio,
				  LPM_BT_WAKE_DEASSERT);
	if (ret < 0) {
	...
	bluesleep_data->host_wake_irq =
	    gpio_to_irq(bluesleep_data->gpio_bt_wake_host.gpio);
...
static int bluesleep_remove(struct platform_device *pdev)
{
	...
	if (test_bit(BT_PROTO, &bs_data->flags)) {
		if (disable_irq_wake(bs_data->host_wake_irq))
			pr_err
			    ("%s: couldn't disable hostwake IRQ wakeup mode\n",
			     __func__);
		if (bs_data->host_wake_irq)
			free_irq(bs_data->host_wake_irq, bs_data);
