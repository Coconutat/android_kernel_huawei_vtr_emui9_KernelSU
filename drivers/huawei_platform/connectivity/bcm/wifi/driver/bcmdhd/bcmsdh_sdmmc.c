...
extern int dw_mci_sdio_wakelog_switch(struct mmc_host *mmc, bool enable);
#endif
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	sdio_quality_add(CMD52_TOTAL, rw, 0);
#endif
...
#endif
#ifdef HW_SDIO_QUALITY_STATISTIC
				sdio_quality_add(CMD52_TOTAL, rw, 1);
#endif
				goto retry;
			}
                                   ...
                                   uint32 *word, uint nbytes)
{
	...
	int word_ret = SDIOH_API_RC_FAIL;
#endif
#ifdef HW_SDIO_RW_RETRY
	int retry_times = 2;
#endif
	...
	DHD_PM_RESUME_RETURN_ERROR(SDIOH_API_RC_FAIL);
#ifdef HW_SDIO_QUALITY_STATISTIC
	sdio_quality_add(CMD53_TOTAL, rw, 0);
#endif
...
#endif
#ifdef HW_SDIO_QUALITY_STATISTIC
		sdio_quality_add(CMD53_TOTAL, rw, 1);
#endif
		goto retry;
	}
...
#endif
#ifdef HW_REG_RECOVERY
	return ((word_ret == 0) ? SDIOH_API_RC_SUCCESS : SDIOH_API_RC_FAIL);
#else
	return ((err_ret == 0) ? SDIOH_API_RC_SUCCESS : SDIOH_API_RC_FAIL);
#endif
}
...
#ifdef CONFIG_MMC_DW_HI3XXX
	return dw_mci_sdio_wakelog_switch(host, enable);
#endif
}
#endif
