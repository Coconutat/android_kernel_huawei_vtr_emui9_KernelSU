...
int wifi_platform_get_fw_path(wifi_adapter_info_t *adapter, unsigned char *buf, int len)
{
	struct wifi_platform_data *plat_data;
	...
	plat_data = adapter->wifi_plat_data;
	if (plat_data->get_fw_path) {
		return plat_data->get_fw_path(buf, len);
	}

	DHD_ERROR(("%s: plat_data->get_fw_path is null\n", __FUNCTION__));
	return -EOPNOTSUPP;
}

int wifi_platform_get_chip_type(wifi_adapter_info_t *adapter, unsigned char *buf, int len)
{
	struct wifi_platform_data *plat_data;
	...
	plat_data = adapter->wifi_plat_data;
	if (plat_data->get_chip_type) {
		return plat_data->get_chip_type(buf, len);
	}

	DHD_ERROR(("%s: plat_data->get_chip_type is null\n", __FUNCTION__));
	return -EOPNOTSUPP;
}
