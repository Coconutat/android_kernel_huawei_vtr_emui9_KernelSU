...
/* =1 use static buf , =0 use dynamic buf */
static int use_static_10k = 1;
#endif
int
dhd_flow_rings_init(dhd_pub_t *dhdp, uint32 num_flow_rings)
{
...
#ifndef BCM_ALLOC_STATIC_10K
	flow_ring_table = (flow_ring_table_t *)MALLOCZ(dhdp->osh, flow_ring_table_sz);
#else
	use_static_10k = 1;
	flow_ring_table = (flow_ring_table_t *)DHD_OS_PREALLOC(dhdp,
		DHD_PREALLOC_IF_FLOW_TBL, flow_ring_table_sz);
	//When use static buf fail, malloc dynamic buf
	if (flow_ring_table == NULL) {
		DHD_ERROR(("%s: flow ring table alloc static failure\n", __FUNCTION__));
		use_static_10k = 0;
		flow_ring_table = (flow_ring_table_t *)MALLOCZ(dhdp->osh, flow_ring_table_sz);
	}
#endif
	...
	for (idx = 0; idx < num_flow_rings; idx++) {
		...
		flow_ring_table[idx].dhd = (void *)dhdp;
#endif
...
#else
	if(use_static_10k == 1)
		DHD_OS_PREFREE(dhdp, flow_ring_table, flow_ring_table_sz);
	else
		MFREE(dhdp->osh, flow_ring_table, flow_ring_table_sz);
#endif
	}
...
#else
		if(use_static_10k == 1) {
			DHD_OS_PREFREE(dhdp, dhdp->flow_ring_table, flow_ring_table_sz);
			dhdp->flow_ring_table = NULL;
		} else {
			MFREE(dhdp->osh, flow_ring_table, flow_ring_table_sz);
		}
...
#ifdef BCM_PATCH_STACKOUT
	//memcpy 1 byte ,avoid stack-out-of-bounds in bcm_mkiovar
	ret = dhd_iovar(dhd, 0, "bus:fl_prio_map", (char *)map, sizeof(uint8),
			NULL, 0, TRUE);
#else
	ret = dhd_iovar(dhd, 0, "bus:fl_prio_map", (char *)map, 4*sizeof(uint8),
			NULL, 0, TRUE);
#endif
	if (ret < 0) {
