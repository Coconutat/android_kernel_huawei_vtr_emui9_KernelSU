...
#ifndef HW_COVERITY_CHECK
		case CC_MC_M1:		return (clock / m1);
		case CC_MC_M1M2:	return (clock / (m1 * m2));
		case CC_MC_M1M2M3:	return (clock / (m1 * m2 * m3));
		case CC_MC_M1M3:	return (clock / (m1 * m3));
#else
		case CC_MC_M1:		return (m1 == 0) ? 0 : (clock / m1);
		case CC_MC_M1M2:	return (m1*m2 == 0) ? 0 : (clock / (m1 * m2));
		case CC_MC_M1M2M3:	return (m1*m2*m3 == 0) ? 0 : (clock / (m1 * m2 * m3));
		case CC_MC_M1M3:	return (m1*m3 == 0) ? 0 : (clock / (m1 * m3));
#endif
		default:		return (0);
		}
	} else {
