...
static ssize_t hall_info_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	char info[256] = { 0 };
...
static ssize_t hall_now_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	packet_data value = 0;
	struct hall_cdev *cdev = dev_get_drvdata(dev);
...
static ssize_t hall_now_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t hall_flag_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	int flag = 0;
	struct hall_cdev *cdev = dev_get_drvdata(dev);
