...
int hw_ocp_lcd_handler(const char *name)
{
	...
	if (hw_lcd_is_default()) {
		HW_OCP_LOG_ERR("%s: panel is not exist, don't handle ocp!.\n", __func__);
		return 0;
	}
...
static int hw_ocp_pmic_register(void)
{
	...
	if (ret) {
		HW_OCP_LOG_ERR("supply_lcd register error.\n");
		return ret;
	}
	ret = hisi_pmic_special_ocp_register(SUPPLY_TP, hw_ocp_handler);
	if (ret) {
		HW_OCP_LOG_ERR("supply_tp register error.\n");
		return ret;
	}
	return ret;
}
