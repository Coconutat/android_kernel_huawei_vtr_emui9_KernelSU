...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
uint16_t crc_ccitt_fast(uint16_t crc, u8* buffer, u32 length)
{
  while (length--)
...
/**
*  SW Protocol Packet Struct
*  byte0  byte1  byte2    byte3  byte4    XX  XX  XX  XX
*   ID    LEN    DevID    crc_H  crc_L   [ data (LEN-5)  ]
*
*/
...
int sw_check_recv_data(const u8 *data, int count)
{
	 ...
	 crc_data = (data[3] << 8) + data[4];

	 crc = crc_ccitt_fast(0,(u8 *)(data+5),count-5);
	 hwlog_debug("crc_ccitt %x  %x \n",crc,crc_data);
	 if(crc!=crc_data)
		 return -1;
...
int hkadc_detect_kb(int kb_detect_adc)
{
    int retry_times = 3;
    int cnt = 0;
    int T_adc;
    ...
    {
        T_adc = hisi_adc_get_value(kb_detect_adc);

        if(T_adc < 0)
        {
            hwlog_err("KB detect ADC read fail!,retry_cnt = %d\n",cnt);
        }
        else
        {
			hwlog_info("KB detect ADC read success!,adc = %d\n",T_adc);
			return T_adc;
        }
    }

    hwlog_err("KB detect read retry 3 times,error!\n");
