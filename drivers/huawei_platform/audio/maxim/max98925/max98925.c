...
static ssize_t max98925_register_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	buf[0] = '\0';
	for (i = 0; i <= 0x38; i++) {
		if(regmap_read(p->regmap, i, &value) < 0) {
			continue;
		}
...
static int max98925_open(struct inode *inode, struct file *filp)
{
	...
	//out driver strenth
	regmap_update_bits(p->regmap, MAX98925_R02B_DOUT_DRV_STRENGTH,M98925_DAI_OUT_DRIVE_MASK,0x00);
...
static int max98925_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	const char *iv_slot_change_name = "iv_slot_change";
	int val = 0;
	...
	ret = of_property_read_u32(i2c->dev.of_node, iv_slot_change_name, &val);
	if (ret !=0) {
		hwlog_err("%s: can't get iv_slot_change from dts, set defaut false value!!!\n", __func__);
		max98925->iv_slot_change = false;
	} else {
		if (val) {
			max98925->iv_slot_change = true;
			hwlog_info("%s: iv_slot_change is true!\n", __func__);
		} else {
			max98925->iv_slot_change = false;
			hwlog_info("%s: iv_slot_change is false!\n", __func__);
		}
	}
