...
#define HWLOG_TAG fan54151_main
...
static int fan54151_charge_init(void)
{
	...
	if (ret)
	{
		hwlog_err("%s: protection_enable fail\n", __func__);
		return -1;
	}
...
static struct loadswitch_ops fan54151_main_ops = {
...
static int fan54151_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = loadswitch_main_ops_register(&fan54151_main_ops);
	if (ret)
	{
		hwlog_err("register loadswitch main ops failed!\n");
