...
static int logbuff_flush_callback(const pkt_header_t *head)
{
	...
	memcpy(pLocalLogBuff + sensorhub_log_buf_rear, pDDRLogBuff + sensorhub_log_buf_rear, flush_size);
	print_stat(6);
...
static ssize_t logbuff_config_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(logbuff_config, S_IWUSR | S_IRUGO, logbuff_config_show, logbuff_config_set);
...
static int sensorhub_logbuff_init(void)
{
	...
	ret = register_mcu_event_notifier(TAG_LOG_BUFF, CMD_LOG_BUFF_ALERT, logbuff_full_callback);
	if (ret) {
		hwlog_err("%s failed register notifier CMD_LOG_BUFF_ALERT:ret %d\n", __func__, ret);
		goto NOTIFY_ERR;
	}

	ret = register_mcu_event_notifier(TAG_LOG_BUFF, CMD_LOG_BUFF_FLUSHP, logbuff_flush_callback);
	if (ret) {
		hwlog_err("%s failed register notifier CMD_LOG_BUFF_FLUSH:ret %d\n", __func__, ret);
		goto NOTIFY_ERR_1;
	}

	pDDRLogBuff = (uint8_t *) ioremap_wc(DDR_LOG_BUFF_ADDR_AP, DDR_LOG_BUFF_SIZE);
	if (!pDDRLogBuff) {
		hwlog_err("%s failed remap log buff\n", __func__);
		goto REMAP_ERR;
	}
	pLocalLogBuff = (uint8_t *) vmalloc(DDR_LOG_BUFF_SIZE);
	if (!pLocalLogBuff) {
