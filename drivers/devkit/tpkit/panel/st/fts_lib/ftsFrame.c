...
int requestFrame(u16 type) {
	...
	while (retry < FRAME_DATA_READ_RETRY) {
		...
		if (ret<OK) {
			TS_LOG_ERR("%s requestFrame:  ERROR %02X\n", __func__, ERROR_I2C_W);
			return ERROR_I2C_W;
		}
	...
	if (answer == type) {
		...
		TS_LOG_ERR("%s The event found has a different type of Frame data ERROR %02X\n", __func__, ERROR_DIFF_COMP_TYPE);
...
int readFrameDataHeader(u16 type, DataHeader *header)
{
	...
	if (readCmdU16(FTS_CMD_FRAMEBUFFER_R, offset, data, FRAME_DATA_HEADER, DUMMY_FRAMEBUFFER) < 0) {
		TS_LOG_ERR("%s  readFrameDataHeader: ERROR %02X\n", __func__, ERROR_I2C_R);
		return ERROR_I2C_R;
	}
	TS_LOG_INFO( "%s Read Data Header done!\n", __func__);

	if (data[0] != FRAME_HEADER_SIGNATURE) {
		TS_LOG_ERR("%s readFrameDataHeader: ERROR %02X The Header Signature was wrong!  %02X != %02X\n",
	...
	TS_LOG_INFO( "%s Type of Frame data OK!\n", __func__);
...
int getMSFrame2(u16 type, MutualSenseFrame *frame)
{
	...
	if (!(type == MS_TOUCH_ACTIVE || type == MS_TOUCH_LOW_POWER || type == MS_TOUCH_ULTRA_LOW_POWER || type == MS_KEY)) {
		TS_LOG_ERR("%s getMSFrame: Choose a MS type of frame data ERROR %02X\n", __func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
...
int getSSFrame2(u16 type, SelfSenseFrame *frame)
{
	...
	if (!(type == SS_TOUCH || type == SS_KEY || type == SS_HOVER || type == SS_PROXIMITY)) {
		TS_LOG_ERR("%s getSSFrame: Choose a SS type of frame data ERROR %02X\n", __func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
	...
	if (temp == NULL) {
		TS_LOG_ERR("%s getSSFrame: temp ERROR %02X\n", __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	...
	if (frame->force_data == NULL) {
		TS_LOG_ERR("%s getSSFrame: frame->force_data ERROR %02X\n", __func__, ERROR_ALLOC);
		kfree(temp);
		return ERROR_ALLOC;
	}
	...
	if (frame->sense_data == NULL) {
		TS_LOG_ERR("%s getSSFrame: frame->sense_data ERROR %02X\n", __func__, ERROR_ALLOC);
		kfree(temp);
