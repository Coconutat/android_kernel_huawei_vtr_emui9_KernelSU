...
info_retry:
	...
	novatek_ts_kit_read(I2C_FW_Address, buf, 17);
	nvt_fw_ver = buf[1];
	x_num = buf[3];
	y_num = buf[4];
	nvt_ts->x_num = x_num;
...
void nvt_kit_read_mdata(uint32_t xdata_addr)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / NVT_TRANSFER_LENGTH); j++) {
			...
			novatek_ts_kit_read(I2C_FW_Address, buf, NVT_TRANSFER_LENGTH + 1);
	...
	for (i = 0; i < (data_len / 2); i++) {
		...
		novatek_ts_kit_read(I2C_FW_Address, buf, (TOUCH_KEY_NUM * 2 + 1));
