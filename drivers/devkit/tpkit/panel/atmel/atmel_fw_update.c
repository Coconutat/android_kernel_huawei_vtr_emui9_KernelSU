 ...
 *
 */
int atmel_diagnostic_command(struct mxt_data *data, u8 cmd, u8 page, u8 index,
...
int atmel_pds_get_data(struct mxt_data *data, struct pds_code *code)
{
	...
	for (i = 0; i < MISC_PDS_DATA_LEN; i++) {//project id 9bytes
		if (code->id[i] < 32 || code->id[i] > 126) {  // ASCII
			code->id[i] = 0;
			break;
		}
	}
...
int atmel_update_fw_file_name(struct mxt_data *data, char *file_name)
{
	...
	if(strlen(data->module_id) == 0) {
		strncpy(data->module_id, "NULL", strlen("NULL"));
	...
	strncpy(data->fw_name, file_name, MAX_FILE_NAME_LENGTH);
...
static u8 mxt_get_bootloader_version(struct mxt_data *data, u8 val)
{
	...
	if (val & MXT_BOOT_EXTENDED_ID) {
		if (atmel_bootloader_read(data, &buf[0], 3) != 0) {
