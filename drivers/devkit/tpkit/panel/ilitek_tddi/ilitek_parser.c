...
/* char M_CFG_NIS = ':'; */
    ...
    if (ERR_ALLOC_MEM(ini_buf)) {
        ilitek_err("Failed to allocate ini_buf memory, %ld\n", PTR_ERR(ini_buf));
        res = -ENOMEM;
        goto out;
    }
    ...
    if (ERR_ALLOC_MEM(tmpSectionName)) {
        ilitek_err("Failed to allocate tmpSectionName memory, %ld\n", PTR_ERR(tmpSectionName));
        res = -ENOMEM;
        goto out;
    }
    ...
    while (true) {
        ...
        if (g_ini_items > PARSER_MAX_KEY_NUM) {
            ilitek_err("MAX_KEY_NUM: Out of length\n");
            goto out;
        }
        ...
        if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
            ...
            if (ini_buf[0] == M_CFG_SSL) {
                ...
                if (ilitek_ini_file_data[g_ini_items].iSectionNameLen > PARSER_MAX_KEY_NAME_LEN) {
                    ilitek_err("MAX_KEY_NAME_LEN: Out Of Length\n");
                ...
                banchmark_flag = 0;
                nodetype_flag = 0;
                ilitek_debug(DEBUG_PARSER, "Section Name: %s, Len: %d\n", tmpSectionName, n - 2);
                continue;
            }
        }
        ...
        if (ilitek_ini_file_data[g_ini_items].iKeyValueLen > PARSER_MAX_KEY_VALUE_LEN) {
            ilitek_err("MAX_KEY_VALUE_LEN: Out Of Length\n");
            res = INI_ERR_OUT_OF_LINE;
            goto out;
        }
...
static int get_ini_key_value(char *section, char *key, char *value)
{
    ...
    for (i = 0; i < g_ini_items; i++) {
        ...
        if (strcmp(key, ilitek_ini_file_data[i].pKeyName) == 0) {
            ...
            ilitek_debug(DEBUG_PARSER, " value:%s , pKeyValue: %s\n", value, ilitek_ini_file_data[i].pKeyValue);
            ret = 0;
            break;
        }
    }
    ...
    if(isspace_t((int)(unsigned char)*s) == 0)
    {
        while((pToken = strsep(&s, ",")) != NULL){
            ...
            if(res == 0)
                ...
                ilitek_info("convert string too long, res = %d\n", res);
            conut++;
        }
    }
...
int core_parser_get_int_data(char *section, char *keyname, char *rv)
{
    ...
    if (rv == NULL || section == NULL || keyname == NULL) {
        ilitek_err("Parameters are invalid\n");
        return -EINVAL;
    }
...
int core_parser_path(char *path)
{
    ...
    if (ERR_ALLOC_MEM(f)) {
        ilitek_err("Failed to open the file at %ld.\n", PTR_ERR(f));
        res = -ENOENT;
        return res;
    }
    ...
    ilitek_info("fsize = %d\n", fsize);
    if (fsize <= 0) {
        ilitek_err("The size of file is invaild\n");
        res = -EINVAL;
        goto out;
    }
    ...
    if (ERR_ALLOC_MEM(tmp)) {
        ilitek_err("Failed to allocate tmp memory, %ld\n", PTR_ERR(tmp));
        res = -ENOMEM;
        goto out;
    }
    ...
    if (res < 0) {
        ilitek_err("Failed to get physical ini data, res = %d\n", res);
        goto out;
    }

    ilitek_info("Parsing INI file doen\n");
