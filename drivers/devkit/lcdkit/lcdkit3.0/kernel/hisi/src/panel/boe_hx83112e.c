...
*panel:boe_hx83112e
*/
static int boe_hx83112e_rgbw_setting(struct hisi_fb_data_type* hisifd, struct lcd_kit_dsi_panel_cmds* rgbw_cmds)
{
	...
	static uint32_t rgbw_total_glim_old = 0;
	int ret = LCD_KIT_OK;
	...
	if ((rgbw_cmds->cmds) && (rgbw_cmds->cmd_cnt>0))
	{
		...
		rgbw_total_glim = (uint32_t)hisifd->de_info.rgbw_total_glim;
		if ((rgbw_saturation_control_old != rgbw_saturation_control) || (frame_gain_limit_old != frame_gain_limit) || \
					...
					(pwm_duty_gain_old != pwm_duty_gain) || (rgbw_total_glim_old != rgbw_total_glim))
		{
			//pwm_duty_gain
			rgbw_cmds->cmds[2].payload[1] = 0x50 + ((pwm_duty_gain >> 8) & 0x03);
			rgbw_cmds->cmds[2].payload[2] = pwm_duty_gain & 0xff;
			LCD_KIT_DEBUG("[RGBW] pwm_duty_gain=%d pwm_duty_gain_old = %d!\n", pwm_duty_gain, pwm_duty_gain_old);
			//pixel_gain_limit
			rgbw_cmds->cmds[2].payload[6] = 0x80 + ((pixel_gain_limit >> 8) & 0x01);
			rgbw_cmds->cmds[2].payload[7] = pixel_gain_limit & 0xff;
			LCD_KIT_DEBUG("[LG-NT36772A-RGBW] pixel_gain_limit=%d pixel_gain_limit_old = %d!\n", pixel_gain_limit, pixel_gain_limit_old);
			//frame_gain_limit
			rgbw_cmds->cmds[2].payload[8] = 0x02 + ((frame_gain_limit >> 8) & 0x01);
			rgbw_cmds->cmds[2].payload[9] = frame_gain_limit & 0xff;
			...
			rgbw_cmds->cmds[2].payload[13] = color_distortion_allowance & 0xff;
			...
			//rgbw total glim
			rgbw_cmds->cmds[2].payload[14] = 0x3E + ((rgbw_total_glim >> 8) & 0x01);
			rgbw_cmds->cmds[2].payload[15] = rgbw_total_glim & 0xff;
			LCD_KIT_DEBUG("[RGBW] rgbw_total_glim=%d rgbw_total_glim_old = %d!\n", rgbw_total_glim, rgbw_total_glim_old);
			//rgbw_saturation_control setting
			rgbw_cmds->cmds[2].payload[18] = 0x40 + ((rgbw_saturation_control >> 8) & 0x03);
			rgbw_cmds->cmds[2].payload[19] = rgbw_saturation_control & 0xff;
		...
		rgbw_total_glim_old = rgbw_total_glim;
	}
...
static int boe_hx83112e_rgbw(struct hisi_fb_data_type* hisifd, int mode)
{
	...
	{
		case RGBW_SET1_MODE :
			ret = boe_hx83112e_rgbw_setting(hisifd, &disp_info->rgbw.mode1_cmds);
			break;
		case RGBW_SET2_MODE :
			ret = boe_hx83112e_rgbw_setting(hisifd, &disp_info->rgbw.mode2_cmds);
			break;
		case RGBW_SET3_MODE :
			ret = boe_hx83112e_rgbw_setting(hisifd, &disp_info->rgbw.mode3_cmds);
			break;
		case RGBW_SET4_MODE :
			ret = boe_hx83112e_rgbw_setting(hisifd, &disp_info->rgbw.mode4_cmds);
			break;
		default:
			ret = LCD_KIT_FAIL;
			break;
