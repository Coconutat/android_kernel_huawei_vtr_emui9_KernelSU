	...
	struct kbase_va_region *region)
{
	...
	if (!err) {
		...
		if (region->flags & KBASE_REG_SCRAMBLE_BIT)
			err = kbase_mmu_insert_pages_with_scramble_bit(kbdev, &kctx->mmu, region->start_pfn + fault_block_pfn,
				&region->cpu_alloc->pages[fault_block_pfn], map_block_size, region->flags, kctx->as_nr, 0);
		else
			err = kbase_mmu_insert_pages(kbdev, &kctx->mmu, region->start_pfn + fault_block_pfn,
				&region->cpu_alloc->pages[fault_block_pfn], map_block_size, region->flags, kctx->as_nr);
	}
		...
		struct kbase_sub_alloc **prealloc_sas)
{
	...
	if (new_pages >= (SZ_2M / SZ_4K)) {
		root_pool = kbase_mem_select_pool(kctx, region->gpu_alloc->lb_policy_id, true);
		KBASE_DEBUG_ASSERT(root_pool);
		*grow_2mb_pool = true;
	} else {
#endif
		root_pool = kbase_mem_select_pool(kctx, region->gpu_alloc->lb_policy_id, false);
		KBASE_DEBUG_ASSERT(root_pool);
		*grow_2mb_pool = false;
#ifdef CONFIG_MALI_2MB_ALLOC
	}
...
int kbase_mmu_insert_pages_with_scramble_bit_no_flush(struct kbase_device *kbdev,
				    struct kbase_mmu_table *mmut,
				    const u64 start_vpfn,
				    struct tagged_addr *phys, size_t nr,
 ...
 * number 'as_nr'.
 */
int kbase_mmu_insert_pages_with_scramble_bit(struct kbase_device *kbdev,
		struct kbase_mmu_table *mmut, u64 vpfn,
		struct tagged_addr *phys, size_t nr,
		unsigned long flags, int as_nr, u64 header_page_number)
{
	...
	err = kbase_mmu_insert_pages_with_scramble_bit_no_flush(kbdev, mmut, vpfn,
