...
void kbasep_cache_clean_worker(struct work_struct *data)
{
	...
	while (kbdev->hwcnt.backend.state == KBASE_INSTR_STATE_CLEANING) {
		...
		{
			int ret;
			ret = wait_event_interruptible_timeout(kbdev->hwcnt.backend.cache_clean_wait,
                                        kbdev->hwcnt.backend.state !=
                                                        KBASE_INSTR_STATE_CLEANING, CACHE_CLEAN_TIMEOUT);
			if(ret <= 0)
			{
#if KBASE_GPU_RESET_EN
				dev_err(kbdev->dev, "Sending reset to GPU as bit stuck issue\n");
