...
MOCKABLE(kbase_pm_check_transitions_nolock) (struct kbase_device *kbdev)
{
	...
	kbdev->pm.backend.desired_l2_state = desired_l2_state;
	...
	if(kbdev->as_stuck_hard_reset) {
		...
		dev_err(kbdev->dev, "l2_available_bitmap= %016llx, powering_on_l2_state= %016llx \n", l2_available_bitmap,kbdev->pm.backend.powering_on_l2_state);
	}
	...
	if (in_desired_state) {
		...
		if(kbdev->as_stuck_hard_reset) {
			dev_err(kbdev->dev, "in_desired_state = %d, desired_tiler_state= %016llx \n",in_desired_state,kbdev->pm.backend.desired_tiler_state);
			dev_err(kbdev->dev, "tiler_available_bitmap= %016llx, powering_on_tiler_state= %016llx \n", tiler_available_bitmap,kbdev->pm.backend.powering_on_tiler_state);
			dev_err(kbdev->dev, "in_desired_state = %d, desired_shader_state= %016llx \n",in_desired_state,kbdev->pm.backend.desired_shader_state);
			dev_err(kbdev->dev, "shader_available_bitmap= %016llx, powering_on_shader_state= %016llx \n", shader_available_bitmap,kbdev->pm.backend.powering_on_shader_state);
		}
...
#define PM_TIMEOUT_AFTER_BIT_STUCK (500)
...
//	timeout = jiffies + PM_TIMEOUT;
	...
	if(kbdev->as_stuck_hard_reset)
	{
		ret = wait_event_interruptible_timeout(kbdev->pm.backend.gpu_in_desired_state_wait,
				kbdev->pm.backend.gpu_in_desired_state, PM_TIMEOUT_AFTER_BIT_STUCK);
	}
	else
	{
		ret = wait_event_interruptible_timeout(kbdev->pm.backend.gpu_in_desired_state_wait,
	...
	if (ret <= 0) {
		...
		dev_err(kbdev->dev, "\tl2_present=%016llx, kbdev->l2_available_bitmap=%016llx\n",
				kbdev->gpu_props.props.raw_props.l2_present, kbdev->l2_available_bitmap);
		dev_err(kbdev->dev, "\tl2_users_count=%x, l2_powered=%x\n",
				kbdev->l2_users_count, kbdev->pm.backend.l2_powered);
		dev_err(kbdev->dev, "\tl2_number_slice=%x \n",
				kbdev->gpu_props.props.l2_props.num_l2_slices);
		dev_err(kbdev->dev, "\tL2=%016llx\n",
				kbdev->pm.backend.desired_l2_state);
		dev_err(kbdev->dev, "\tpoweroff_wait_in_progress = %d\n",
				kbdev->pm.backend.poweroff_wait_in_progress);
		dev_err(kbdev->dev, "\tpoweroff_is_suspend = %d\n",
				kbdev->pm.backend.poweroff_is_suspend);
		dev_err(kbdev->dev, "\tpowering_on_shader_state = %016llx\n",
				kbdev->pm.backend.powering_on_shader_state);
		dev_err(kbdev->dev, "\tpowering_on_tiler_state= %016llx\n",
				kbdev->pm.backend.powering_on_tiler_state);
		dev_err(kbdev->dev, "\tpowering_on_l2_state = %016llx\n",
