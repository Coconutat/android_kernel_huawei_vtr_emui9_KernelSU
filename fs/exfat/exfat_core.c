...
s32 ffsWriteFile(struct inode *inode, FILE_ID_T *fid, void *buffer, u64 count, u64 *wcount)
{
	...
	while (count > 0) {
		...
		if ((offset == 0) && (oneblkwrite == p_bd->sector_size)) {
			...
			memcpy(((char *) tmp_bh->b_data)+offset, ((char *) buffer)+write_bytes, (s32) oneblkwrite); /* [false alarm]:Linux original code */
			if (sector_write(sb, LogSector, tmp_bh, 0) != FFS_SUCCESS) {
				brelse(tmp_bh);
				goto err_out;
			}
...
s32 exfat_alloc_cluster(struct super_block *sb, s32 num_alloc, CHAIN_T *p_chain)
{
	...
	if (hint_clu == CLUSTER_32(~0)) {
		...
		printk(KERN_ERR "hint_clu = %d, ERROR\n", hint_clu);
		return -1;
	}
...
s32 load_alloc_bitmap(struct super_block *sb)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		if (FAT_read(sb, clu.dir, &(clu.dir)) != 0 || 0 == clu.dir || cluster_count ++ > p_fs->num_clusters)
			return FFS_MEDIAERR;
	}
		...
		for (i = 0; i < p_bd->sector_size && index <= 0xFFFF; i += 2) {
			...
			if (skip) {
				...
				if (upcase_table[col_index] == NULL) {
					DPRINTK("alloc = 0x%X\n", col_index);
					upcase_table[col_index] = (u16 *) kmalloc(UTBL_ROW_COUNT * sizeof(u16), GFP_KERNEL);/*[false alarm]:native code*/
					if (upcase_table[col_index] == NULL) {
						ret = FFS_MEMORYERR;
						goto error;
					}
...
ENTRY_SET_CACHE_T *get_entry_set_in_dir(struct super_block *sb, CHAIN_T *p_dir, s32 entry, u32 type, DENTRY_T **file_ep)
{
	...
	DPRINTK("es sec %llu offset %d flags %d, num_entries %u buf ptr %pK\n",
...
static s32 __write_partial_entries_in_entry_set(struct super_block *sb, ENTRY_SET_CACHE_T *es, sector_t sec, s32 off, u32 count)
{
	...
	DPRINTK("es %pK sec %llu off %d count %d\n", es, (unsigned long long)sec, off, count);
	num_entries = count;
			...
			if ((entry_type == TYPE_UNUSED) || (entry_type == TYPE_DELETED)) {
				...
				if ((entry_type == TYPE_FILE) || (entry_type == TYPE_DIR)) {
					...
					if (is_feasible_entry) {
						...
						/* it is impossible that order is zero 
						 * while is_feasible_entry is true, 
						 * there must be something wrong with media data 
						 */
						if (order == 0) {
