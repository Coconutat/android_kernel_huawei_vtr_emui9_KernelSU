			   ...
			   unsigned short pageofs)
{
	...
	/* temp shortcut for pageofs = 0 */
	if (!pageofs && nr_pages == 1) {
		if (compressed_pages[0] == pages[0])
			return 0;
		if (pages[0]) {
			copy_highpage(pages[0], compressed_pages[0]);
			return 0;
		}
	}
...
extern int z_erofs_unzip_lz4(void *in, void *out,
			     size_t inlen, size_t outlen, bool accel);
				  ...
				  bool accel)
{
	...
	ret = z_erofs_unzip_lz4(vin, vout + pageofs,
		clusterpages * PAGE_SIZE, outlen, accel);
