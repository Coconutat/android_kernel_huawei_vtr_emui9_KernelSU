...
void hi6402_hs_mbhc_on(struct snd_soc_codec *codec)
{
	struct hi6402_platform_data *priv = snd_soc_codec_get_drvdata(codec);

	if (!priv) {
		pr_err("%s : cannot get drv data\n", __FUNCTION__);
		return;
	}
	...
	/* open mbhc vref */
	hi64xx_update_bits(codec, HI6402_MBHD_VREF_REG, 1 << HI64xx_MBHC_VREF_BIT, 0);
	snd_soc_write(codec,  HI6402_SAR_CFG_REG, 0x7C);
	/* mbhc on */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG, 1 << HI64xx_MBHC_ON_BIT, 0);
	msleep(30);
...
unsigned int hi6402_get_voltage_value(struct snd_soc_codec *codec, unsigned int voltage_coefficient)
{
	int retry = 3;
	...
	/* saradc on */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG, 0x1 << HI6402_SARADC_PD_BIT, 0);
	/* start saradc */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG,
			0x1<<HI6402_SAR_START_BIT, 0x1<<HI6402_SAR_START_BIT);

	while(retry--) {
		usleep_range(1000, 1100);
		if (hi64xx_check_saradc_ready_detect(codec)) {
			sar_value = snd_soc_read(codec, HI6402_SAR_VALUE_REG);
	...
	/* end saradc */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG, 0x1<<HI6402_SAR_START_BIT, 0);
	/* saradc pd */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG,
			0x1 << HI6402_SARADC_PD_BIT, 0x1 << HI6402_SARADC_PD_BIT);
...
void hi6402_hs_enable_hsdet(struct snd_soc_codec *codec, struct hi64xx_mbhc_config mbhc_config)
{
	snd_soc_write(codec, HI6402_ANA_HSD_CTRL_REG, mbhc_config.hs_ctrl);
	if(mbhc_config.hs_detect_extern_cable)
		snd_soc_write(codec, HI6402_ANA_MBHD_VREF_REG, mbhc_config.hs_mbhc_vref_reg_value);
	else
		snd_soc_write(codec, HI6402_ANA_MBHD_VREF_REG, 0x9E);
}

void hi6402_hs_mbhc_off(struct snd_soc_codec *codec)
{
	/* eco off */
	hi64xx_update_bits(codec, HI6402_MIC_DISCHG_REG,  1 << HI64xx_MICBIAS_ECO_ON_BIT, 0);
	pr_info("%s : eco disable \n", __FUNCTION__);

	/* vref off */
	hi64xx_update_bits(codec, HI6402_ANA_MBHD_VREF_REG, 1 << HI64xx_MBHC_VREF_BIT,
			1 << HI64xx_MBHC_VREF_BIT);

	/* mbhc cmp off */
	hi64xx_update_bits(codec, HI6402_ANA_MIC_PD_REG, 1 << HI64xx_MBHC_ON_BIT,
			1 << HI64xx_MBHC_ON_BIT);
}

static struct hs_mbhc_reg hi6402_hs_mbhc_reg = {
	.irq_source_reg = 0x2000701c,
	.irq_mbhc_2_reg = 0x20007016,
};

static struct hs_mbhc_func hi6402_hs_mbhc_func = {
	.hs_mbhc_on = hi6402_hs_mbhc_on,
	.hs_get_voltage = hi6402_get_voltage_value,
	.hs_enable_hsdet = hi6402_hs_enable_hsdet,
	.hs_mbhc_off = hi6402_hs_mbhc_off,
};

static struct hi64xx_hs_cfg hi6402_hs_cfg = {
	.mbhc_reg = &hi6402_hs_mbhc_reg,
	.mbhc_func = &hi6402_hs_mbhc_func,
	.res_detect_func = NULL,
};
...
* MAX VALUE is 18
*/
static DECLARE_TLV_DB_SCALE(ep_pga_tlv, -2100, 150, 0);//lint !e570
...
static int hi6402_utils_init(struct hi6402_platform_data *pd)
{
	...
	ret = hi64xx_utils_init(pd->codec, pd->cdc_ctrl, &cfg, pd->resmgr, HI64XX_CODEC_TYPE_6402);
...
static int hi6402_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = hi64xx_mbhc_init(codec, hi6402_pd->node, &hi6402_hs_cfg,
