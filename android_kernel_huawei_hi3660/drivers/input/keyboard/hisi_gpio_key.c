...
static int hisi_gpio_key_probe(struct platform_device* pdev)
{
	...
	if (err) {
		support_smart_key = 0;
		printk(KERN_INFO "%s: Not support smart_key\n", __FUNCTION__);
	} else {
	...
	if (gpio_is_valid(gpio_key->gpio_up)) {
		err = devm_gpio_request(&pdev->dev, (unsigned int)gpio_key->gpio_up, "gpio_up");
		if (err < 0) {
		...
		if (err < 0) {
			dev_err(&pdev->dev ,"Failed to set gpio_up directoin!\n");
			goto err_get_gpio;
		}
	...
	if (gpio_is_valid(gpio_key->gpio_down)) {
		err = devm_gpio_request(&pdev->dev, (unsigned int)gpio_key->gpio_down, "gpio_down");
		if (err) {
			dev_err(&pdev->dev, "Fail request gpio:%d\n", gpio_key->gpio_down);
			goto err_get_gpio;
		}
		...
		if (err < 0) {
			dev_err(&pdev->dev ,"Failed to set gpio_down direction!\n");
			goto err_get_gpio;
		}
