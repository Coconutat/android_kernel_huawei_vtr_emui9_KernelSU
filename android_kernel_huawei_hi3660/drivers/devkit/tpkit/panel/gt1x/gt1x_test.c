		...
		struct gt1x_ts_test *ts_test)
{
	...
	if (have_bus_error) 
		gt1x_strncat(ts_test->test_info, "0F-", TS_RAWDATA_RESULT_MAX);
	else
		gt1x_strncat(ts_test->test_info, "0P-", TS_RAWDATA_RESULT_MAX);

	for (i = 1; i < MAX_TEST_ITEMS; i++) {
		/* if have tested, show result */
		if (ts_test->test_result[i]) {
			if (GTP_TEST_PASS == ts_test->test_result[i])
				gt1x_strncatint(ts_test->test_info, i, "%dP-", TS_RAWDATA_RESULT_MAX);
			else 
				gt1x_strncatint(ts_test->test_info, i, "%dF-", TS_RAWDATA_RESULT_MAX);
		}
	...
	if (ts_test->rawdata.size) {
		...
		gt1x_data_statistics(
				...
				statistics_data,
				STATISTICS_DATA_LEN);
		gt1x_strncat(ts_test->test_info, statistics_data,TS_RAWDATA_RESULT_MAX);
	} else {
		TS_LOG_INFO("NO valiable rawdata\n");	
		gt1x_strncat(ts_test->test_info, "[0,0,0]",TS_RAWDATA_RESULT_MAX);
	}
	...
	if (have_bus_error){
		gt1x_strncat(ts_test->test_info, "-software_reason", TS_RAWDATA_RESULT_MAX);
	}
	else if (have_panel_error) {
		gt1x_strncat(ts_test->test_info, "-panel_reason", TS_RAWDATA_RESULT_MAX);
	}

	gt1x_strncat(ts_test->test_info, "-GT", TS_RAWDATA_RESULT_MAX);
	gt1x_strncat(ts_test->test_info, ts_test->ts->hw_info.product_id, TS_RAWDATA_RESULT_MAX);
	gt1x_strncat(ts_test->test_info, "-", TS_RAWDATA_RESULT_MAX);
	gt1x_strncatint(ts_test->test_info, ts_test->ts->dev_data->ts_platform_data->panel_id,"%d;",TS_RAWDATA_RESULT_MAX);
