...
#define WRITE_STORE_CMD_MIN_LENGTH 3

#ifdef SCRIPTLESS


unsigned int st_data[CMD_RESULT_STR_LEN] = {0};
				...
				const char *buf, size_t count)
{
	int ret;
	struct fts_ts_info *info = fts_get_info();
	unsigned char pAddress[FTS_PADRESS_LEN] = {0};
	unsigned int byte_count = 0;
	int i ;

	unsigned int data[FTS_PADRESS_LEN] = {0};
	...
	byte_count = data[FTS_PADRESS_LEN - 1];
	if (byte_count < 5 || byte_count >= FTS_PADRESS_LEN) {
		TS_LOG_ERR("%s: byte count less than 5, ERROR\n", __func__);
		return -EINVAL;
	}
				...
				const char *buf, size_t count)
{
	...
	if(byte_count >8 || byte_count < 2) {
#ifdef SCRIPTLESS_DEBUG
		TS_LOG_ERR("%s : Byte count abnormal\n",__func__);
#endif
		return count;
	}
#ifdef SCRIPTLESS_DEBUG
...
#endif
	if (ret)
		TS_LOG_ERR("%s:Unable to read register\n", __func__);
	return count;
}
				...
				const char *buf, size_t count)
{
	...
	unsigned int byte_count =0 ;
	int i ;
	memset(st_data, 0x00, ARRAY_SIZE(st_data));
	memset(pAddress_i2c, 0x00, ARRAY_SIZE(pAddress_i2c));
	memset(info->cmd_write_result, 0x00, ARRAY_SIZE(info->cmd_write_result));
	/*skip the bracket between cmds*/
	sscanf(buf, "%x %x", st_data ,(st_data + 1));
	byte_count = st_data[0] << 8 | st_data[1];

	if (byte_count <= ARRAY_SIZE(pAddress_i2c)) {
		for(i=0; i < (byte_count) ;i++) {
			/*skip spaces the between cmds*/
			sscanf(&buf[3*(i+2)] , "%x ", (st_data+i));
		}
	} else {
#ifdef SCRIPTLESS_DEBUG
		TS_LOG_INFO("%s : message size is more than allowed limit of 512 bytes\n",__func__);
#endif
		snprintf(info->cmd_write_result, sizeof(info->cmd_write_result), "{Write NOT OK}\n");
		return -EINVAL;
	}
#ifdef SCRIPTLESS_DEBUG
	TS_LOG_INFO("%s: Byte_count=  %02d| Count = %02d | size of buf:%02d\n",
			__func__,byte_count,(int)count,(int)sizeof(buf));
	printk("%s: Input Data 1:",__func__);
	for(i =0 ; i < byte_count; i++) {
		printk(" %02X",st_data[i]);
		pAddress_i2c[i] = (unsigned char)st_data[i];
	}
...
#endif
	/*0xb3  0xb1 is i2c write cmd*/
	if((pAddress_i2c[0] == 0xb3)&&(pAddress_i2c[3] == 0xb1)) {
		if (byte_count < WRITE_STORE_CMD_MIN_LENGTH) {
			TS_LOG_ERR("%s: byte_count abnormal\n", __func__);
			return -EINVAL;
		}
		ret = fts_writeCmd(pAddress_i2c, WRITE_STORE_CMD_MIN_LENGTH);
		msleep(20);
		ret = fts_writeCmd(&pAddress_i2c[3], byte_count-WRITE_STORE_CMD_MIN_LENGTH);
	} else {
		ret = fts_writeCmd(pAddress_i2c, byte_count);
	}

#ifdef SCRIPTLESS_DEBUG
	TS_LOG_INFO("%s:DATA :", __func__);
	for(i=0; i < byte_count; i++) {
...
static DEVICE_ATTR(iread, 0660, NULL, fts_i2c_read_store);
static DEVICE_ATTR(iread_result, 0440, fts_i2c_read_show, NULL);
static DEVICE_ATTR(iwr, 0660, NULL, fts_i2c_wr_store);
static DEVICE_ATTR(iwr_result, 0440, fts_i2c_wr_show, NULL);
static DEVICE_ATTR(iwrite, 0660, NULL, fts_i2c_write_store);
static DEVICE_ATTR(iwrite_result, 0440, fts_i2c_write_show, NULL);
