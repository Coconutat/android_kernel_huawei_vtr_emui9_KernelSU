...
static int imx286hybird_do_hw_reset(hwsensor_intf_t* si, int ctl, int id)
{
        ...
        void  *argp)
{
    ...
    switch(data->cfgtype){
        ...
        case SEN_CONFIG_RESET_HOLD:
            ret = imx286hybird_do_hw_reset(si, CTL_RESET_HOLD, data->mode);
            break;
        case SEN_CONFIG_RESET_RELEASE:
            ret = imx286hybird_do_hw_reset(si, CTL_RESET_RELEASE, data->mode);
            break;
        default:
