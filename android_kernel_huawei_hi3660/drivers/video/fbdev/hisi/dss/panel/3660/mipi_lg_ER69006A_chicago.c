	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "LG_eR69006A 6' CMD TFT Chicago\n");
...
static int mipi_lg_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, DTS_COMP_LG_ER69006);
	if (!np) {
	...
	if (g_lcd_fpga_flag == 1) {
		...
		gpio_lcd_vcc3v1 = of_get_named_gpio(np, "gpios", 5);	//gpio_7_1
		gpio_lcd_vcc1v8 = 49;
	} else {
	...
	pinfo->type =PANEL_MIPI_CMD;// lcd_display_type;
...
static const struct of_device_id hisi_panel_match_table[] = {
	{
		.compatible = DTS_COMP_LG_ER69006,
		.data = NULL,
	},
	{},
};
