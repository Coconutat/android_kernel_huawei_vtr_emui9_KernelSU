...
#include "mipi_sharp_TD4322_6p0.h"
...
static int mipi_sharp_panel_on_old(struct platform_device *pdev)
{
	struct hisi_fb_data_type *hisifd = NULL;
...
static int mipi_sharp_panel_on_new(struct platform_device *pdev)
{
	struct hisi_fb_data_type *hisifd = NULL;
	struct hisi_panel_info *pinfo = NULL;
	char __iomem *mipi_dsi0_base = NULL;
...
static int mipi_sharp_panel_on(struct platform_device *pdev)
{
	...
	if (g_new_sequence_supported) {
		ret = mipi_sharp_panel_on_new(pdev);
	} else {
		ret = mipi_sharp_panel_on_old(pdev);
	}
	return ret;
}
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "SHARP_TD4322 6' CMD TFT\n");
	...
	const char *buf, size_t count)
{
	...
	if (flag==CE_OFF){
		g_ce_mode =0;
		mipi_dsi_cmds_tx(sharp_ce_off_cmds, \
		ARRAY_SIZE(sharp_ce_off_cmds),\
		mipi_dsi0_base);
	} else  if (flag==CE_SRGB_MODE) {
		g_ce_mode=1;
		mipi_dsi_cmds_tx(sharp_ce_srgb_cmds, \
		ARRAY_SIZE(sharp_ce_srgb_cmds),\
		mipi_dsi0_base);
	} else if (flag==CE_USER_MODE){
		g_ce_mode=2;
		mipi_dsi_cmds_tx(sharp_ce_user_cmds, \
		ARRAY_SIZE(sharp_ce_user_cmds),\
		mipi_dsi0_base);
	} else if (flag==CE_VIVID_MODE){
		g_ce_mode=3;
		mipi_dsi_cmds_tx(sharp_ce_vivid_cmds, \
		ARRAY_SIZE(sharp_ce_vivid_cmds),\
		mipi_dsi0_base);
	}
...
static ssize_t mipi_sharp_panel_lcd_reg_read_show(struct platform_device *pdev,
	char *buf)
{
...
static ssize_t mipi_sharp_panel_lcd_reg_read_store(struct platform_device *pdev,
	const char *buf, size_t count)
{
	int ret = 0;
	unsigned long val = 0;
...
static struct hisi_fb_panel_data g_panel_data = {
	...
	.lcd_reg_read_show = mipi_sharp_panel_lcd_reg_read_show,
	.lcd_reg_read_store = mipi_sharp_panel_lcd_reg_read_store,
...
static const struct of_device_id hisi_panel_match_table[] = {
	{
		.compatible = DTS_COMP_SHARP_TD4322_6P0,
		.data = NULL,
	},
	{},
};
