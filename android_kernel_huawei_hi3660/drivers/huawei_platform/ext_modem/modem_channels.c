...
int check_software_flow_control(unsigned int t_chan_num)
{
	...
	do {
		remain_time =
		    wait_event_timeout(p_mdm_chan->m_sw_fc_wtq,
				       (SW_FLOW_CTRL_DISABLE ==
					atomic_read(&p_mdm_chan->m_sw_fc_stat)
					|| (0 == get_modem_bus_state())),
				       msecs_to_jiffies
				       (WAIT_SW_FLOW_CTRL_TIMEOUT));

		if (0 == get_modem_bus_state()) {
			hwlog_err
			    ("%s %d: channel %d wait software contrl disable but VIA CBP reset happened!\n",
			     __func__, __LINE__, p_mdm_chan->m_chan_num);
			ret = -1;
			break;
		} else if (0 == remain_time) {
			hwlog_err
			    ("%s %d: channel %d wait CBP software flow control disable timeout in 2s.\n",
			     __func__, __LINE__, p_mdm_chan->m_chan_num);
			failed_times++;

			if (failed_times >= MAX_WAIT_SW_FLOW_CTRL_TIMES) {
				snprintf(rst_buf, CBP_EXCEPT_STACK_LEN - 1,
					 "%s %d: fail to wait channel %d software flow control disable, notify VIA to produce ramdump\n",
					 __func__, __LINE__,
					 p_mdm_chan->m_chan_num);
				hwlog_err
				    ("%s %d: fail to wait channel %d software flow control disable, notify VIA to produce ramdump\n",
				     __func__, __LINE__,
				     p_mdm_chan->m_chan_num);
 ...
 */
static int modem_channel_write(unsigned int t_chan_num,
			       const unsigned char *pt_payload_buf, int t_count)
{
	int ret = 0;
	...
	if ((MODEM_CHANNEL_AT_PC_NUM == t_chan_num)
		...
		apcp_current_subpacket_printk((char *)pt_payload_buf, t_count,
					      1, t_chan_num);
	}

	ret =
	    kfifo_in_locked(&g_modem_chans.pm_channels[t_chan_num]->
			    m_transmit_fifo, pt_payload_buf, t_count,
			    &g_modem_chans.pm_channels[t_chan_num]->
			    m_write_lock);
	...
	queue_work(g_modem_chans.pm_channels[t_chan_num]->pm_write_wq,
		   &g_modem_chans.pm_channels[t_chan_num]->m_write_work);
	return ret;
}
...
static void modem_channel_write_work(struct work_struct *work)
{
	...
	fifo_size =
	    kfifo_out_locked(&p_mdm_chan->m_transmit_fifo,
			     p_mdm_chan->pm_trans_buffer, count,
			     &p_mdm_chan->m_write_lock);
	...
	ret =
	    modem_write_packets(p_mdm_chan->pm_trans_buffer, cur_data_len,
				t_chan_num);
	...
	if ((count < MODEM_TTY_WAKEUP_CHARS)
	    && (t_chan_num > MODEM_CHANNEL_CONTROL_NUM)) {
...
int modem_rnic_write(unsigned char *pt_rnic_buf, int t_count)
{
	...
	ret =
	    modem_write_rnic_packets(pt_rnic_buf, t_count,
				     MODEM_CHANNEL_RNIC_NUM);
 ...
 */
int modem_tty_write_to_channel(unsigned int t_tty_index,
			       const unsigned char *pt_tty_buf, int t_count)
{
	int ret = -1;
	int wr_count = 0;
...
static void ctrl_msg_analyze(char *pt_buf)
{
	...
	if (pt_buf) {
		...
		t_msg_id =
		    (ctrl_chan_msg.m_id_hi << 8) + ctrl_chan_msg.m_id_low;

		switch (t_msg_id) {
		case FLOW_CONTROL_MSG_ID:
			p_mdm_chan =
			    g_modem_chans.pm_channels[ctrl_chan_msg.m_chan_num];

			if (ctrl_chan_msg.m_chan_num >=
			    MODEM_CHANNEL_CONTROL_NUM
			    && ctrl_chan_msg.m_chan_num < MODEM_CHANNEL_NR) {
				if (SW_FLOW_CTRL_ENABLE ==
				    ctrl_chan_msg.m_option) {
					hwlog_info
					    ("%s %d: channel %d soft flow ctrl enable!\n",
					     __func__, __LINE__,
					     p_mdm_chan->m_chan_num);
					atomic_set(&p_mdm_chan->m_sw_fc_stat,
						   SW_FLOW_CTRL_ENABLE);
				} else if (SW_FLOW_CTRL_DISABLE ==
					   ctrl_chan_msg.m_option) {
					hwlog_info
		...
		default:
			hwlog_err
			    ("%s %d: unknow control message received, message is %d\n",
 ...
 */
int modem_channel_push(void *pt_pkt_payload_buf, unsigned int t_chan_num,
		       int t_payload_len, unsigned char tranHi)
{
	...
	if (NULL == pt_pkt_payload_buf) {
		hwlog_err("%s %d: NULL pointer! pt_pkt_payload_buf=0x%lx\n",
			  __func__, __LINE__,
			  (unsigned long)pt_pkt_payload_buf);
		return -1;
	}
	...
	if ((MODEM_CHANNEL_AT_PC_NUM == chan_num)
		...
		apcp_current_subpacket_printk(pt_pkt_payload_buf, payload_len,
					      0, chan_num);
	}

	if (chan_num > MODEM_CHANNEL_CONTROL_NUM
	    && chan_num < MODEM_CHANNEL_RNIC_NUM) {
...
void modem_channels_reset(void)
{
	...
	for (index = 0; index < MODEM_CHANNEL_NR; index++) {
		...
		hwlog_info("%s %d: modem channel %d reset.\n", __func__,
			   __LINE__, p_mdm_chan->m_chan_num);
	}
...
static int each_channel_init(modem_channel_t *pt_chan)
{
	...
	sprintf(pt_chan->m_arr_work_name, "%s%d", pt_chan->pm_name,
		pt_chan->m_chan_num);
	pt_chan->pm_write_wq =
	    create_singlethread_workqueue(pt_chan->m_arr_work_name);
...
int modem_channels_init(void)
{
	...
	for (index = MODEM_CHANNEL_CONTROL_NUM; index < MODEM_CHANNEL_NR;
	     index++) {
		p_chan = g_modem_chans.pm_channels[index] =
		    kzalloc(sizeof(modem_channel_t), GFP_KERNEL);
