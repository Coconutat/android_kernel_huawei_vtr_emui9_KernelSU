...
static int hall_gpio_get_value(int gpio_type, int gpio)
{
	int value = 0;
...
static int ak8789_get_data(struct hall_device *h_dev)
{
	...
	if (h_type & 0x01) {
		if (!h_dev->h_info.auxiliary_io[0]) {
			p_data |=
			    !hall_gpio_get_value(hall_gpio_type, h_dev->h_info.
					    gpio[single_north_pole]);
		} else
		    if (!hall_gpio_get_value(hall_gpio_type,
			h_dev->h_info.auxiliary_io[single_north_pole])) {
			p_data |=
			    !hall_gpio_get_value(hall_gpio_type, h_dev->h_info.
					    gpio[single_north_pole]);
		} else {
	...
	if (h_type & 0x02) {
		if (!h_dev->h_info.auxiliary_io[1]) {
			p_data |= (!hall_gpio_get_value(hall_gpio_type,
			     h_dev->h_info.gpio[single_south_pole])) << 1;
		} else
		    if (!hall_gpio_get_value(hall_gpio_type,
			h_dev->h_info.auxiliary_io[single_south_pole])) {
			p_data |= (!hall_gpio_get_value(hall_gpio_type,
			     h_dev->h_info.gpio[single_south_pole])) << 1;
		} else {
...
static int ak8789_get_info(struct hall_cdev *cdev, char *buf)
{
	...
	list_for_each_entry(temp_dev, &data->head, list) {
		if (max_length > 0) {
			ret = snprintf(temp_buf, max_length,
				     "[hall]id:%d; type:%d;wakeup:%d; x_coor:%d; y_coor:%d\n",
				     temp_dev->hall_id, temp_dev->hall_type,
...
static int ak8789_get_status(struct hall_cdev *cdev)
{
	...
	list_for_each_entry(temp_dev, &data->head, list) {
		...
		pdata |= temp_data << (temp_dev->
				  hall_id ? (1 << temp_dev->hall_id) : 0);
	}
...
{
	return ak8789_register_report_data(50);
}
...
static void ak8789_set_report_value(struct hall_cdev *cdev)
{
	...
	if (!cdev->enable) {
		hwlog_info
		    ("test switch is off, open the switch if need to test\n");
		return;
	}
...
static void ak8789_report_delayed_work(struct work_struct *work)
{
	...
	struct hall_device *h_dev =
	    container_of(work, struct hall_device, h_delayed_work.work);
	struct hall_device *temp_dev;
				     ...
				     struct device_node *node)
{
	...
	if (ret) {
		...
		if (auxiliary_flag & 0x01) {
			ret =
			    of_property_read_u32(node, HALL_AUXILIARY_N,
						 &hall_dev->h_info.
						 auxiliary_io[0]);
			if (ret) {
				hwlog_err
				    ("Failed to get auxiliary io; ret:%d\n",
				     ret);
				/*goto error;*/
			}
		} else {
		...
		if (auxiliary_flag & 0x02) {
			ret =
			    of_property_read_u32(node, HALL_AUXILIARY_S,
						 &hall_dev->h_info.
						 auxiliary_io[1]);
			if (ret) {
				hwlog_err
	...
	ret = of_property_read_u32(node, HALL_GPIO_TYPE, &hall_dev->hall_gpio_type);
	if (ret)
...
static irqreturn_t ak8789_event_handler(int irq, void *hall_dev)
{
	...
	irq_time =
	    (now.tv_sec -
	     h_dev->h_info.last_time[index].tv_sec) * HALL_1000000 +
...
static void hall_interrupt_abnormity_work(struct work_struct *work)
{
	struct ak8789_data *data =
	    container_of(work, struct ak8789_data, inter_work);
	struct hall_device *temp_dev;
	int count = 0;
	int hall_type;
	int check_result = 0;
				...
				struct device_node *node)
{
	...
	if (hall_type & 0x01) {
		...
		ret =
		    snprintf(hall_dev->h_info.name[0], HALL_MAX_STRING_LEN,
			     "hall%d_north", hall_dev->hall_id);
		if (ret < 0) {
	...
	if (hall_type & 0x02) {
		...
		ret =
		    snprintf(hall_dev->h_info.name[1], HALL_MAX_STRING_LEN,
			     "hall%d_south", hall_dev->hall_id);
		if (ret < 0) {
...
static int ak8789_request_irq(struct hall_device *hall_dev)
{
	...
	hall_gpio_type = hall_dev->hall_gpio_type;
	if (hall_type & 0x01) {
	...
	if (hall_type & 0x02) {
		...
		if(hall_gpio_type & 0x01){
			...
			ret = request_threaded_irq(irq, NULL, ak8789_event_handler,
				trigger_val | wakeup_flag | IRQF_ONESHOT,
				hall_dev->h_info.name[1], hall_dev);
		} else {
			trigger_val = gpio_get_value(gpio) ?
				IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH;
			ret = request_irq(irq, ak8789_event_handler,
	...
	hwlog_info("irq done[0:%d][1:%d]\n", hall_dev->h_info.irq[0],
		   hall_dev->h_info.irq[1]);
...
static int ak8789_probe(struct platform_device *pdev)
{
	...
	data = (struct ak8789_data *)devm_kzalloc(&pdev->dev,
					       sizeof(struct ak8789_data),
					       GFP_KERNEL);
	if (!data) {
...
/* ak8789 init_data */
	...
	for_each_child_of_node(node, temp) {
		...
		if (default_state) {
			count++;
			hall = devm_kzalloc(&pdev->dev,
					 (sizeof(struct hall_device)),
					 GFP_KERNEL);
			if (!hall) {
				hwlog_err
			...
			INIT_DELAYED_WORK(&hall->h_delayed_work,
					  ak8789_report_delayed_work);

			ret = ak8789_get_common_configs(hall, temp);
			if (ret) {
				hwlog_err
				    ("Failed to read dts cfg, ret = %d\n", ret);
				goto free_hall_dev;
			}
			ret = snprintf(hall->name, HALL_MAX_STRING_LEN,
				     "hall%d\n", hall->hall_id);
			if (ret < 0) {
...
static int ak8789_remove(struct platform_device *pdev)
{
	...
	while (!list_empty(&data->head)) {
		data_dev =
		    list_first_entry(&data->head, struct hall_device, list);
