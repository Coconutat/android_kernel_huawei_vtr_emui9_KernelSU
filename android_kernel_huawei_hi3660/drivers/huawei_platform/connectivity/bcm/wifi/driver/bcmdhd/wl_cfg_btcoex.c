...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
	struct mutex iovar_mutex;
	struct mutex reg_iovar_mutex;
#endif
};
...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
static void btcoex_lock_local(void)
{
	if (!btcoex_info_loc) {
		WL_ERR(("btcoex_info_loc is NULL, no lock\n"));
		return;
	}

	mutex_lock(&(btcoex_info_loc->iovar_mutex));
}

static void btcoex_unlock_local(void)
{
	if (!btcoex_info_loc) {
		WL_ERR(("btcoex_info_loc is NULL, no unlock\n"));
		return;
	}

	mutex_unlock(&(btcoex_info_loc->iovar_mutex));
}

static void btcoex_reg_lock_local(void)
{
	if (!btcoex_info_loc) {
		WL_ERR(("btcoex_info_loc is NULL, no reg lock\n"));
		return;
	}

	mutex_lock(&(btcoex_info_loc->reg_iovar_mutex));
}

static void btcoex_reg_unlock_local(void)
{
	if (!btcoex_info_loc) {
		WL_ERR(("btcoex_info_loc is NULL, no reg unlock\n"));
		return;
	}

	mutex_unlock(&(btcoex_info_loc->reg_iovar_mutex));
}
#endif
...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
	ret = (wldev_ioctl(dev, WLC_SET_VAR, ioctlbuf_local, sizeof(ioctlbuf_local), true));
...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
	ret = (dev_wlc_bufvar_set(dev, name, (char *)&reg_addr[0], sizeof(reg_addr)));

	btcoex_reg_unlock_local();
...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
#define MAX_BTCPARAMS_ATTEMPTS 10
#endif
...
static int set_btc_esco_params(struct net_device *dev, bool trump_sco)
{
	...
	if (trump_sco) {
		...
		do {
			...
			if ((!dev_wlc_intvar_get_reg(dev, "btc_params", regaddr, &reg50)) ) {
				WL_TRACE(("reg50=%x, saved_reg50=%x\n", reg50, saved_reg50));
			} else {
				WL_TRACE(("!!! Can't read back btc_params\n"));
			}

			if (reg50 == saved_reg50) {
				WL_TRACE(("reg50 check pass. 0x%x=0x%x.\n", reg50, saved_reg50));
				break;
			}
		} while (try_cnt++ < MAX_BTCPARAMS_ATTEMPTS);
#else
		regaddr = 50;
		dev_wlc_intvar_set_reg(dev, "btc_params",
			(char *)&regaddr, (char *)&saved_reg50);
...
#ifdef HW_DHCP_BTCOEX_RESTORE_FAIL
	mutex_init(&btco_inf->iovar_mutex);
	mutex_init(&btco_inf->reg_iovar_mutex);
#endif
