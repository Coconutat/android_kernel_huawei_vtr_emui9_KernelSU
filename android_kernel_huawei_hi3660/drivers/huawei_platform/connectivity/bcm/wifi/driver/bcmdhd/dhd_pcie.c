...
{
	volatile uint8 data;
#if defined(BCM47XX_ACP_WAR) || defined(PCIE_RW_WITH_MB)
...
#ifdef DHD_DEVWAKE_EARLY
extern int dhd_wlan_dev_wake_value(void);
...
dhdpcie_bus_suspend(struct dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		DHD_GENERAL_UNLOCK(bus->dhd, flags);
#ifdef DHD_DEVWAKE_EARLY
		if (bus->dhd->devwake_enable && !bus->dhd->deepsleep) {
			DHD_ERROR(("%s: wifi chip is not deepsleep state when enter suspend.\n", __FUNCTION__));
		}
#endif /* DHD_DEVWAKE_EARLY */
...
#ifdef DHD_DEVWAKE_EARLY
		if (bus->dhd->devwake_enable) {
			int dev_wake_pin = dhd_wlan_dev_wake_value();
			if (dev_wake_pin > 0) {
				DHD_ERROR(("%s:Entering resume devwake pin is high, reset the devwake pin\n", __FUNCTION__));
				dhd_wlan_dev_wake(0);
			}
			//Force the deepsleep flag to 0 to sync FW state
			bus->dhd->deepsleep = 0;
		}
#endif /* DHD_DEVWAKE_EARLY */
		/* Resume */
...
/** Add bus dump output to kernel log directly **/
void dhd_bus_dump_local(dhd_pub_t *dhdp)
{
	uint16 flowid;
	int ix = 0;
	flow_ring_node_t *flow_ring_node;
	...
	DHD_ERROR(("PCIe bus dump:\n"));
	DHD_ERROR(("h2d_mb_data_ptr_addr 0x%x, d2h_mb_data_ptr_addr 0x%x\n",
		dhdp->bus->h2d_mb_data_ptr_addr, dhdp->bus->d2h_mb_data_ptr_addr));
	DHD_ERROR(("%s %4s %2s %4s %4s %4s %10s",
		"Num:", "Flow", "If", "Prio", "Qlen", "CLen",
		"Overflows\n"));
	for (flowid = 0; flowid < dhdp->num_flow_rings; flowid++) {
		flow_ring_node = DHD_FLOW_RING(dhdp, flowid);
		if (flow_ring_node->active) {
			flow_info = &flow_ring_node->flow_info;
			DHD_ERROR(("%3d. %4d %2d %4d %4d %4d %10u RD:%x WR:%x\n", ix++,
				...
				dhdp->bus->ring_sh[flowid+BCMPCIE_D2H_COMMON_MSGRINGS].ring_state_w,
				dhdp->bus->ring_sh[flowid+BCMPCIE_D2H_COMMON_MSGRINGS].ring_state_r));
		}
	}
	DHD_ERROR(("D3 inform cnt %d\n", dhdp->bus->d3_inform_cnt));
	DHD_ERROR(("D0 inform cnt %d\n", dhdp->bus->d0_inform_cnt));
	DHD_ERROR(("D0 inform in use cnt %d\n", dhdp->bus->d0_inform_in_use_cnt));
	DHD_ERROR(("D3 Ack WAR cnt %d\n", dhdp->bus->d3_ack_war_cnt));
}
#endif
...
#define TEL_HUAWEI_NV_WITXL2G5G0_NUMBER      384
...
#define TEL_HUAWEI_NV_WITXL2G5G0_NAME        "WITXL0"
...
static int hw_load_radio_calibrate_data(char * calbufp, uint size, uint nv_number, uint nv_length, char *nv_name)
{
	int ret = 0;
	...
	info.nv_number  = nv_number;//nve item
	info.valid_size = nv_length;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, nv_name, 7);
...
static int hw_calibrate_nvram_vars(struct dhd_bus *bus, char * bufp, uint len)
{
	...
	CalDateLen = hw_load_radio_calibrate_data(CalDateBuf, CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WINVRAM_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WINVRAM_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WIRXNVRAM_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WIRXNVRAM_NAME);
#ifdef SUPPORT_WT_WIFI_MIMO_CALIBRATE
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXCCK_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXCCK_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXC1_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXC1_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXBW0_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXBW0_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXBW1_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXBW1_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXL2G5G0_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXL2G5G0_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXL2G5G1_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXL2G5G1_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
					TEL_HUAWEI_NV_WITXOFFSET2G_NUMBER, TEL_HUAWEI_NV_WINVRAM_LENGTH, TEL_HUAWEI_NV_WITXOFFSET2G_NAME);
	CalDateLen += hw_load_radio_calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE,
