...
static struct loadswitch_ops fan54151_aux_ops = {
...
static int fan54151_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = request_irq(di->irq_int, fan54151_interrupt, IRQF_TRIGGER_FALLING, "loadswitch_int_irq_aux", di);
	if (ret)
	{
	...
	ret = loadswitch_aux_ops_register(&fan54151_aux_ops);
	if (ret)
	{
		hwlog_err("register loadswitch aux ops failed!\n");
