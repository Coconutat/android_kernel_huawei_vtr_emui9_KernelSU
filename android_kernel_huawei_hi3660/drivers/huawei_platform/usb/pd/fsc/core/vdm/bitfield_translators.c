...
AmaVdo getAmaVdo(FSC_U32 in) {
    ...
    ret.cable_hw_version =          (in >> CABLE_HW_VERSION_OFFSET) & CABLE_HW_VERSION_MASK;
    ret.cable_fw_version =          (in >> CABLE_FW_VERSION_OFFSET) & CABLE_FW_VERSION_MASK;
    ret.vdo_version =               (in >> VDO_VERSION_OFFSET) & VDO_VERSION_MASK;
    ret.vconn_full_power =          (in >>  VCONN_FULL_POWER_OFFSET) & VCONN_FULL_POWER_MASK;
    ret.vconn_requirement =         (in >>  CVONN_REQUIREMENT_OFFSET) & CVONN_REQUIREMENT_MASK;
    ret.vbus_requirement =          (in >>  VBUS_REQUIREMENT_OFFSET) & VBUS_REQUIREMENT_MASK;
    ret.usb_ss_supp =               (in >>  USB_SS_SUPP_OFFSET) & USB_SS_SUPP_MASK;
