...
#define VENC_MSG_RESP_INPUT_DONE        (VENC_MSG_RESP_BASE + 0x7)
#define VENC_MSG_RESP_OUTPUT_DONE       (VENC_MSG_RESP_BASE + 0x8)
...
#define VENC_MUTEX_INIT(lock) \
 do { \
     mutex_init(lock); \
 } while(0)

#define VENC_MUTEX_LOCK(lock) \
 do { \
     mutex_lock(lock); \
 } while(0)

#define VENC_MUTEX_UNLOCK(lock) \
 do { \
     mutex_unlock(lock); \
 } while(0)
