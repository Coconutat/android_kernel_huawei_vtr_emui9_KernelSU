...
static inline HI_S32 DRV_PROC_Create_Node(DRV_PROC_FUNC *proc, HI_CHAR *proc_name, drv_proc_read_fn pfn_read, drv_proc_write_fn pfn_write)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 0)
    entry = create_proc_entry(proc->proc_name, 0644, HI_NULL);
    if (!entry)
    {
        return HI_FAILURE;
    }
    ...
    entry->data       = proc;
#else
    entry = proc_create_data(proc->proc_name, 0644, HI_NULL, &gProcFopS, (void*)proc);
    if (!entry)
    {
        return HI_FAILURE;
    }
...
static inline HI_S32 DRV_PROC_Destroy_Node(DRV_PROC_FUNC *proc)
{
    ...
    memset(proc, 0, sizeof(DRV_PROC_FUNC)); /* unsafe_function_ignore: memset */
