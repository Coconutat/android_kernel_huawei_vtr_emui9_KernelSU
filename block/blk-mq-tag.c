		...
		busy_tag_iter_fn *fn, void *priv)
{
	...
	int ret = -EPERM;
	if (tags->set && tags->set->hisi_tagset_ops && tags->set->hisi_tagset_ops->tagset_all_tag_busy_iter_fn)
		ret = tags->set->hisi_tagset_ops->tagset_all_tag_busy_iter_fn(tags, fn, priv);
	if (ret) {
		if (likely(ret == -EPERM)) {
